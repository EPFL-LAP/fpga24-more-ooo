...
To generate the results of the experiment that studies the effect of varying the number of tags N on our generated circuit for the ```bicg``` benchmark (Table 1 in the paper), run the ```N_tag_simulate_experiment.sh``` script to generate the simulation results (i.e., cycle count).
```
cd Dynamatic/etc/dynamatic/fpga24-more-ooo/experiments
...
The outcome of the above step is the generation of a report (bicg_N_sim_results.rpt) containing the simulation time and the cycle count for each N, as reported in the paper, in the Dynamatic/etc/dynamatic/fpga24-more-ooo/experiments/simulation_reports_N_tags/ directory.
