...
// AYA: 05/08/2023: added the following function to parse the tagged field
static bool readTagged(DFnetlist_Impl& DF, blockID id, Agnode_t* v) 
{
    const char* attr = agget(v, (char *) "tagged");
...
//            setChannelFrequency(c, 1);
		...
		// AYa: added the following two lines
		blockID src_component_id = getSrcBlock(c);
		blockID dst_component_id = getDstBlock(c);

		// AYA: 07/11/2021: Commented the following lines to identify if a channel is inner or outer in my proper way!!
        // inner/outer channel
		...
		////////////// Aya commented until here!! ANd added the following lines
		if(src == dst) {
			// inner channel
			freq = BBG.getFrequency(src);
		} else {
			// outer channel (i.e., between 2 different BBs)
				// it's either between two consecutive BBs, following the CFG,
					// OR in BBs not following the CFG!!
			bbArcID connection = BBG.findArc(src, dst);
			if(connection != invalidDataflowID) {
				freq = BBG.getFrequencyArc(connection);
			} else {
				// this means it's not following the CFG!!!
				// The frequency of executing the edge is = to the frequency of executing the dst BB, unless the edge is not always taken!!
				if(src_type == BRANCH && (dst_type == MUX || dst_type == MERGE)) {
					bool is_true_BB = false;

				// 1st) identify if the dst is at the true or false side of the branch, 2nd) identify the BB arc at the true or false by searching for a branch in the same BB that preserves the control order! 
					...
					if(src_true_channel == c) {
						is_true_BB = true;
					} else {
						assert(src_false_channel == c);
						is_true_BB = false;
					}
			
					// Now search for a Branch in the same BB as the Src, but has edges compatible with the control flow graph!
					ForAllBlocks(b) {
						...
						if(temp_type == BRANCH && temp_basic_block == src) {
							...
							// Check the same for the False edge!
		        			...
		        			if(true_connection != invalidDataflowID && false_connection != invalidDataflowID) {
								if(is_true_BB) {
									// get the frequency of the true arc
									bbArcID connection = BBG.findArc(src, true_dst);
									assert(connection != invalidDataflowID);
									freq = BBG.getFrequencyArc(connection);
								} else {
									// get the frequency of the false arc
									bbArcID connection = BBG.findArc(src, false_dst);
									assert(connection != invalidDataflowID);
									freq = BBG.getFrequencyArc(connection);
								}
							}
			...
			/////////////////////////////////////////////////////////////////////////////
					...
					// Otherwise, the channel is guaranteed to execute as long as we arrived at the destination BB!!!??? Needs checking!!
					freq = BBG.getFrequency(dst);
				} 
			}
	...
	// AYA 11/11/2021: added this to debug the channel frequencies
	cout << "\n===================================\n";
	cout << "PRINTING CHANNELS FREQUENCIES" << endl;
	ForAllChannels(c) {
		double freq = getChannelFrequency(c);
		...
		cout << "Freq of channel in Src_BB number " << src_BB << " and dst_BB number " << dst_BB << " is::: " << freq << endl;

	}
	cout << "\n===================================\n";
	/////////////////////// End of Aya's debugging
