...
map<BlockType,string> DFnetlist_Impl::BlockType2String = {
    ...
    {SYNCH, "Synch"},
    {TAGGER, "Tagger"}, // AYA: 17/09/2023
    {UNTAGGER, "Un_Tagger"},
    {ALIGNER_BRANCH, "Aligner_Branch"}, // AYA: 30/09/2023
    {ALIGNER_MUX, "Aligner_Mux"}, // AYA: 30/09/2023
    {TMFO, "TMFO"}, // Aya: 14/05/2023
    {ROB, "ROB"},  // Aya: 07/08/2023
    {FREE_TAGS_FIFO, "Free_Tags_Fifo"}  
};
...
// Aya: the following is the constructor that is actually called in our main_shab in test.cpp
...
bool DFnetlist_Impl::isInnerChannel(channelID c) {
	// AYA: 07/11/2021: commented the original implementation and introduced a new one!!!
...
// AYA: 05/08/2023
void DFnetlist_Impl::setBlockTagged(blockID id, bool is_tagged)
{
    assert(validBlock(id));
...
// AYA: 05/08/2023
bool DFnetlist_Impl::getBlockTagged(blockID id)
{
    return blocks[id].is_tagged;
}
