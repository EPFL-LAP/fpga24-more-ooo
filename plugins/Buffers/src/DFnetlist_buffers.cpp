...
// The following Buffer's version is Aya's FPGA'23
...
//gian variable for multithreading buff placement
bool multithread = is_tag_from_input(); 

//const int hardcode_N = 1;
int hardcode_N = number_of_tags_from_input();

bool weirdo = false;  // AYA: 06/10/2023
...
bool DFnetlist_Impl::addElasticBuffersBB(double Period, double BufferDelay, bool MaxThroughput, double coverage, int timeout, bool first_MG) {
    ...
    if (MaxThroughput) {
        ...
        for (int i = 0; i < optimize_num; i++) {
			// AYA: THIS TELLS THAT MILP IS AFFECTED BOTH BY THE MG (which is filled by the getMGfromCFDFC function that I developed AND THE CHANNEL FRQUENCIES (which I modified in computeChannelFrequencies) and MGfreq!!!
...
bool DFnetlist_Impl::addElasticBuffersBB_sc(double Period, double BufferDelay, bool MaxThroughput, double coverage, int timeout, bool first_MG) {
    ...
    if (MaxThroughput) {
        ...
        cout << "Extracting marked graphs" << endl;
        coverage = extractMarkedGraphsBB(coverage);
		cout << "Aya:Done Extracting marked graphs!\n\n";
    }
    ...
    // AYA: 28/02/2023: added the new LOOPMUX
    ForAllChannels(c) {
        if (getBlockType(getSrcBlock(c)) == MUX || (getBlockType(getSrcBlock(c)) == LOOPMUX)
            ...
            || getBlockType(getSrcBlock(c)) == CNTRL_MG) {   // AYA: 08/12/2023: Added extra condition to treat CMerges of FPGA'24 like Muxes of FPL'22
            
            // AYA: 14/12/2023: added the following condition:
            if(multithread) {
                setChannelTransparency(c, 1);
                setChannelBufferSize(c, hardcode_N);
            } else {
                setChannelTransparency(c, 1);
        ...
        if (getBlockType(getSrcBlock(c)) == FORK && getBlockType(getDstBlock(c)) == OPERATOR) {
            ...
            // THEN I NEED TO REMOVE THE BX!! Currently, I remove it by implementing the module as a pass through wires in elastic_components.vhd
        }
        ///////////////////////////////////
    ...
    // Aya: debugging: 06/12/2023
    cout << "AYA: size of MC_disjoint is: " << MG_disjoint.size() << endl;
    ...
    for (int i = 0; i < MG_disjoint.size(); i++) {
		...
		// AYA: The execution reached here, then at milp.solve, it crashed!!!!
			// checking the initializations of the milp object!!!
        ...
        Milp_Model::Status stat = milp.getStatus();
        if (stat != Milp_Model::OPTIMAL and stat != Milp_Model::NONOPTIMAL) {
            cout << "AYAA: No solution found to add elastic buffers." << endl;
            setError("No solution found to add elastic buffers.");
            return false;
        }
        ...
        if (MaxThroughput) {
         ...
         // Aya: debugging: 06/12/2023
        cout << "AYA: right after the if(MaxThroughput) condition!! " << "\n";
        ...
        for (channelID c: buffers) {
            ...
            setChannelTransparency(c, transparent);
            /*Gian add a slot more if it is transparent*/
            if(transparent)
            { 
                // AYA: 16/12/2023: added the following extra condition
        ...
        // Aya: debugging: 06/12/2023
        cout << "AYA: right before the if(multithread) condition!! " << "\n";

        int N = 0;
        if(multithread)
        {
            if(weirdo) {
                 // AYA: 04/10/2023: COMMENTING THIS BECAUSE GIAN'S FIND_N_MULTITHREAD GOES INTO INFINITE LOOP IN SOME CASES!!
                int new_N = 0;
                for (auto sub_mg: components[i]) {
                    new_N = find_N_multithread(MG[sub_mg], true);
                    if(N == 0 || new_N < N) // take smallest N (correctness reasons)
                        N = new_N;
                }
                // AYA: 20/09/2023: commented out the following because I do not know its purpose!!!

            } else {
                N = hardcode_N;  // AYA: 04/10/2023: futher to commenting the above, I need to manually specify this N!!
            }   
...
// Aya: note that we filled the structure milpVarsEB &Vars after calling the function "createMilpVarsEB_sc"
...
bool DFnetlist_Impl::createThroughputConstraints(Milp_Model& milp, milpVarsEB& Vars, bool first_MG)
{
    ...
    for (int mg = 0; mg < MG.size(); ++mg) {
        ...
        //int N = find_N_multithread(MG[mg], false); //+1 just to be sure
          int N = hardcode_N;//6;  // AYA: 04/10/2023: commented the above and replaced it with a hardcore that should be adjusted based on the example
        if(N <= 0)
            N = 1;
        }
...
bool DFnetlist_Impl::createThroughputConstraints_sc(Milp_Model &milp, milpVarsEB &Vars, int mg, bool first_MG) {
    ...
    for (auto sub_mg: components[mg]) {
        ...
        if(multithread){
            if(weirdo) {
                 // AYA: 04/10/2023: COMMENTING THIS BECAUSE GIAN'S FIND_N_MULTITHREAD GOES INTO INFINITE LOOP IN SOME CASES!!
                N = find_N_multithread(MG[sub_mg], false);
                // AYA: 20/09/2023: commented this because I do not know its purpose!!
            } else {
                N = hardcode_N; //  AYA: 04/10/2023: further to above, hardcoding N specific to each example
            }

            // if(N > 40) //simple threshold
            //     N = 40;
            //N = 4;
            if(N <= 0)
                N = 1;
