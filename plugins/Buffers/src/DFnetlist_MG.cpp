...
double DFnetlist_Impl::extractMarkedGraphsBB(double coverage) {
    ...
    while (covered_freq < coverage * total_freq) {
		...
		// Aya: this step seems to be correct
...
DFnetlist_Impl::subNetlistBB DFnetlist_Impl::extractMarkedGraphBB(const map<bbID, double> &freq) {
    ...
    for (bbArcID i = 0; i < BBG.numArcs(); i++){
		// Aya: skip if the BB has no arc?
        if (BBG.getSrcBB(i) == 0 || BBG.getDstBB(i) == 0) continue;
		// Aya: skip if the frequency of that edge is <= 0
        if (N_e[i] <= 0) continue;
...
DFnetlist_Impl::subNetlist DFnetlist_Impl::getMGfromCFDFC(Dataflow::DFnetlist_Impl::subNetlistBB &BB_CFDFC) {
    ...
    ForAllChannels(i) {
        ...
        if (!isInnerChannel(i)) {
           			// AYA, I'm guessing that this is what causing the problem in my case since I could have two components connected in BBs that are not connected in the CFG!! My DFG is not strictly following the CFG!

			// AYA: commented the following three lines (including the condition)
            //bbArcID connection = BBG.findArc(src, dst);
            //assert(connection != invalidDataflowID);
            //if (BB_CFDFC.hasBasicBlockArc(connection)) {

			// AYA: added the following logic to decide when an edge should be included and when not!
			bool flag = false;
			bool is_true_BB = false;  // it's true if the src is a branch and the dst is at the true side of this branch and false if it's at the false side of the branch!
			
			blockID src_component_id = getSrcBlock(i);
			bbID src_basic_block = getBasicBlock(src_component_id);

			blockID dst_component_id = getDstBlock(i);
			
			BlockType src_type = getBlockType(src_component_id);
			BlockType dst_type = getBlockType(dst_component_id);

			
			if(src_type == BRANCH && (dst_type == MERGE || dst_type == MUX)) {
				// identify if the dst is at the true or false side of the src Branch!
				portID src_true_port = getTruePort(src_component_id);
				portID src_false_port = getFalsePort(src_component_id);
				assert(src_true_port != invalidDataflowID && src_false_port != invalidDataflowID);
				channelID src_true_channel = getConnectedChannel(src_true_port);
				channelID src_false_channel = getConnectedChannel(src_false_port);
				if(src_true_channel == i) {
					is_true_BB = true;
				} else {
					assert(src_false_channel == i);
					is_true_BB = false;
				}
			
				// 1) loop over all components in search for a Branch, in the same BB as the src_type, with at least one of the 2 edges compatible with the CFG edge!
				// 2) check if any of this Branch edges are to a BB that's part of the cycle
				// 3) Identify if this BB is at the true or false side of the Branch
				// 4) Check if the dst is at the true or false side of the src Branch and take it only if it's compatible with 3)!!
				ForAllBlocks(b) {
						BlockType temp_type = getBlockType(b);
						bbID temp_basic_block = getBasicBlock(b);
						if(temp_type == BRANCH && temp_basic_block == src) {
							// check if it this branch has an edge that is also an edge in the BB_CFDFC 
							portID true_port = getTruePort(b);
							portID false_port = getFalsePort(b);
							assert(true_port != invalidDataflowID && false_port != invalidDataflowID);
							// checking first the true edge
							channelID true_channel = getConnectedChannel(true_port);
							blockID true_dst_block = getDstBlock(true_channel);
							bbID true_dst = getBasicBlock(true_dst_block);
						
							bbArcID true_connection = BBG.findArc(temp_basic_block, true_dst);
							// check if this edge is compatible with the CFG
		        			if(true_connection != invalidDataflowID) {
								// check if this edge is part of the cycle!!
								if (BB_CFDFC.hasBasicBlockArc(true_connection)) {
									// So, the trueBB of this Branch is included in the cycle.. We will include the channel under study if it's at the true side of the src Branch
									if(is_true_BB) {
										flag = true;
									}   // note that the default of flag is false, so if nothing triggers it to be true, it'll remain false

								}

							}

							// Check the same for the False port!
							channelID false_channel = getConnectedChannel(false_port);
							blockID false_dst_block = getDstBlock(false_channel);
							bbID false_dst = getBasicBlock(false_dst_block);
						
							bbArcID false_connection = BBG.findArc(temp_basic_block, false_dst);
							// check if this edge is compatible with the CFG
		        			if(false_connection != invalidDataflowID) {
								// check if this edge is part of the cycle!!
								if (BB_CFDFC.hasBasicBlockArc(false_connection)) {
									// flag will be true if the original src is having the dst at its false side!
									if(!is_true_BB) {
										flag = true;
									}
				...
				// in any other case, we always take any channel connecting any two components given that the src and dst BBs are part of the cycle!!
				flag = true;
			}
