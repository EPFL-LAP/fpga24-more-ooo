...
public:
	...
	static int bx_buffer_wrapper_id;   // AYA: 14/09/2022: a new component to act as a wrapper for the Buffer that is used inside the Bx (sequentializer of the smart LSQ interface)

	static int bx_join_wrapper_id;

	static int lazy_fork_id;
	...
	static int tagger_id;

	static int un_tagger_id;

	static int free_tags_fifo_id;

	static int aligner_branch_id;
	static int aligner_mux_id;
    ...
    void addLoopsExits_ConsInProdOut(ENode* producer, ENode* consumer, std::vector<int>& modif_consBB_deps);
    ...
    void TEMP_deleteExtraPhis(networkType network_flag);  // AYA: 09/11/2022: temporary for experimenting the REGENERATE
    ...
    void addFork();
    void Aya_addFork();
    void extraCheck_AddBranchSuccs();
    ...
    void connectReturnToVoid_irredundant();
...
/////////////////////////////////////////////////
	...
	void constructDomTree(Function& F);
    ...
    */
    ...
    void addMemoryInterfaces(const bool useLSQ, bool is_smart_cntrlOrder_flag, bool lazy_fork_flag);
    ...
    void fix_ST_MC_interfaces(bool fix_mc_st_interfaces_flag_yes_extra_ST_output);  // implemented in AddCtrl.cpp
    void remove_MC_ST_counter(ENode* mc_node);
    void deliver_ACK_tokens_wrapper();
    void deliver_ACK_tokens_cond_st();
    void deliver_ACK_tokens_loop_st();
    void WRONG_deliver_ACK_tokens();
    void fixForks();

    // AYA: 26/07/2023: added the following functions to have a single SUPPRESS in the case of more prod than cons that is followed by a consumer that is not guaranteed to execute
    void fixSuppress_more_prod_than_cons_wrapper(networkType network_flag);
    void fixSuppress_more_prod_than_cons(ENode* producer, ENode* consumer, ENode* branch, ENode* branch_cond, std::ofstream& dbg_file, bool is_true_succs);
    void insert_more_prod_cons_supp_mux(ENode* branch, ENode* branch_cond, ENode* new_extra_cond, BasicBlock* target_bb, bool is_negated_new_extra_cond);
	...
	// Aya: 12/09/2022: broke down the following function into two to allow for two modes of interfacing with the LSQ
	//void addControlForLSQ();
	void add_smart_ControlForLSQ(bool lazy_fork_flag);
	void add_naive_ControlForLSQ();
	bool is_common_loop(std::ofstream& dbg_file, BBNode* bb_1, BBNode* bb_2);

	// represents 1 node (group) in the graph G 
	struct Group {
		/*
			An object of this type holds the idx of where to insert a Bx component and what are the BBs containing the producer and consumer Bxs to my Bx 
		*/
		...
		// 31/10/2022: added a new field to hold the actual_producer ENode of each group.. it is a vector in case the group has multiple predecessors!
			// This field is filled inside the applyGSA_Loops_simplified function and is read inside the addLoopBranches_postGSA function
		std::vector<ENode*> actual_producers;

		bool operator ==(const int& rhs_first_bb_idx) const {
			return(rhs_first_bb_idx == first_bb_idx);
		}
	...
	void identifyMemDeps(std::ofstream& dbg_file, ENode* lsq_enode, std::vector<prod_cons_mem_dep>& all_mem_deps); // called inside add_smart_ControlForLSQ
	void identifyGroupIdxs(ENode* lsq_enode); // called inside add_smart_ControlForLSQ
	void constructGroupsGraph(ENode* lsq_enode, std::vector<Group>& groups, const std::vector<prod_cons_mem_dep>& all_mem_deps);
	void printDebugGroupsGraph(std::ofstream& dbg_file, const std::vector<Group>& groups, ENode* lsq_enode, const std::vector<prod_cons_mem_dep>& all_mem_deps);
	void translateGroups_toCircuit(ENode* lsq_enode, const std::vector<Group>& groups, bool lazy_fork_flag);
	void returnCommonLoopHeaders(BBNode* bb_1, BBNode* bb_2, std::vector<BBNode*>& common_loop_headers);
	void removeUselessBxs(bool lazy_fork_flag);
	void applyGSA_Loops_simplified(std::ofstream& dbg_file, ENode* lsq_enode, std::vector<Group>& groups, bool lazy_fork_flag);
	void applyGSA_Non_Loops_simplified(std::ofstream& dbg_file, ENode* lsq_enode, const std::vector<Group>& groups, bool lazy_fork_flag);
	void OLDapplyGSA_Non_Loops_simplified(std::ofstream& dbg_file, const std::vector<Group>& groups);

	void addLoopBranches_postGSA(std::ofstream& dbg_file, ENode* lsq_enode, const std::vector<Group>& groups, bool lazy_fork_flag);
	ENode* addLoopBranches_postGSA_singleLoop(std::ofstream& dbg_file, ENode* phi_node, ENode* producer);

	void old_applyGSA_Non_Loops_simplified(std::ofstream& dbg_file, const std::vector<Group>& groups);

	// AYA: 27/11/2022
	void convert_Merges_to_CMerges();

	//bool compareByFirstBBIndex(const Group& group_1, const Group& group_2);
	void applyTransitiveReduction(std::vector<Group>& groups);
	void addJoin(bool lazy_fork_flag);

	void convert_LSQ_fork_to_lazy();

	void printDebugDominatorTree();
	...
	void constructSOP(std::ofstream& dbg_file, BBNode* startBB, BBNode* bbnode, const std::vector<int>& modif_BB_deps, std::vector<std::vector<pierCondition>>& f_sop, bool experiment_flag = false);
	...
	// Aya: added on 06/09/2022: but currently still not used!
	bool calculate_F_Gen(std::ofstream& dbg_file, const std::vector<std::vector<pierCondition>>& f_prod, const std::vector<std::vector<pierCondition>>& f_cons, std::vector<std::vector<pierCondition>>& f_gen_simplified);
	...
	// 05/08/2023:  added the following functions to label some components inside loops as tagged 
	void tag_some_components();
	void fix_tagged_inputs();
	void insert_loop_exit_robs();
	void addFork_LoopMux_tagOutput();
	...
	// 27/09/2023:
	void insert_tagger_untagger_if_else_node(ENode* if_else_master_cmerge);
	...
	// AYA: 24/02/2023
	// New functions added for the new logic of loop management
