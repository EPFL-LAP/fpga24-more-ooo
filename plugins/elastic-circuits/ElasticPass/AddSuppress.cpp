 ...
 * @param 
 */
void CircuitGenerator::fixSuppress_more_prod_than_cons_wrapper(networkType network_flag) {
	std::ofstream dbg_file;
	dbg_file.open("fixSuppress_more_prod_than_cons.txt");

	node_type branch_type; // is it a Branch_c or Branch_n
	std::string branch_name; 

	ENode* branch;
	ENode* branch_cond;
	std::vector<ENode*>* branch_true_succs;
	std::vector<ENode*>* branch_false_succs;
	...
	for(auto& enode: *enode_dag) {
		if(enode->type != branch_type) 
			continue;  // skip if it is not a Branch

		branch = enode;
		switch(network_flag) {
			case data:
				branch_cond = branch->CntrlPreds->at(1);
				branch_true_succs = branch->true_branch_succs;
				branch_false_succs = branch->false_branch_succs;
			break;

			case constCntrl:
			case memDeps:
				branch_cond = branch->CntrlPreds->at(0);
				if(branch->true_branch_succs_Ctrl->size() + branch->false_branch_succs_Ctrl->size() > 0) {
					branch_true_succs = branch->true_branch_succs_Ctrl;
					branch_false_succs = branch->false_branch_succs_Ctrl;
				} else {
					assert(branch->true_branch_succs_Ctrl_lsq->size() + branch->false_branch_succs_Ctrl_lsq->size() > 0);
					branch_true_succs = branch->true_branch_succs_Ctrl_lsq;
					branch_false_succs = branch->false_branch_succs_Ctrl_lsq;
				}
		}

		// Check if the Branch is in a loop exit BB or if its condition is that of a loop exit (the latter check is extra in case the SUPPRESS was inserted in a BB that is a loop body)
		if(BBMap->at(enode->BB)->is_loop_exiting || BBMap->at(branch_cond->BB)->is_loop_exiting || 
						(branch_cond->type == Phi_ && BBMap->at(branch_cond->CntrlPreds->at(0)->BB) ) ) {
			// now check if the producer that this Branch is fed from can execute while the consumer that the branch feeds does not execute (forward path check that does not consider any backward edges)
			ENode* producer = branch->producer_to_branch;
			ENode* consumer = nullptr;

			// loop over the true succs of the Branch and check if any of them does not postdominate the producer_to_branch
			for(int i = 0; i < branch_true_succs->size(); i++) {
				if(!my_post_dom_tree->properlyDominates(branch_true_succs->at(i)->BB, producer->BB)) {
					consumer = branch_true_succs->at(i);
					fixSuppress_more_prod_than_cons(producer, consumer, branch, branch_cond, dbg_file, true); 
				}
			}
			
			// repeat the same for the false succs
			for(int i = 0; i < branch_false_succs->size(); i++) {
				if(!my_post_dom_tree->properlyDominates(branch_false_succs->at(i)->BB, producer->BB)) {
					consumer = branch_false_succs->at(i);
					fixSuppress_more_prod_than_cons(producer, consumer, branch, branch_cond, dbg_file, false); 
				}
			}
...
void CircuitGenerator::fixSuppress_more_prod_than_cons(ENode* producer, ENode* consumer, ENode* branch, ENode* branch_cond, std::ofstream& dbg_file, bool is_true_succs) {
	...
	ENode* supp_condition_enode = nullptr;
	...
	suppress_flag = calculate_F_Supp(dbg_file, f_prod, f_cons, f_supp);

	if(suppress_flag) {
		supp_condition_enode = applyShannon(dbg_file, dbg_file, producer->BB, f_supp, is_negated_cond);
		// 1st: identify if the current condition of the Branch suppresses when it is true or when it is false,
			/*
				This should be known as follows:
					 1) if the consumer is connected in the true succs, then it DOES NOT suppress when the condition is true
					 2) if the consumer is connected in the false succs, then it suppresses when the condition is true and DOES NOT suppress when false
					 3) if the is_negated of the condition in the pred of the Branch is false, follow 1) and 2) as is
					 4) But, if the is_negated of the condition in the pred of the Branch is true, do the opposite of 1) and 2)
			*/
		insert_more_prod_cons_supp_mux(branch, branch_cond, supp_condition_enode, consumer->BB, is_negated_cond);
	}

}

// AYA: TODO: MAKE SURE TO CHECK THE SIGNS OF THE CONDITIONS IN THIS FUNCTION (AND THE VALUE OF THE CONSTANT THAT FEEDS THE MUX FOR INSTANCE!!!)
void CircuitGenerator::insert_more_prod_cons_supp_mux(ENode* branch, ENode* branch_cond, ENode* new_extra_cond, BasicBlock* target_bb, bool is_negated_new_extra_cond) {

	// create a MUX that will take as its SEL the branch_cond, as its in0 new_extra_cond and as its in1 a constant 1
	...
	new_mux->CntrlPreds->push_back(branch_cond);
	// replace the branch in the succs of its branch_cond with the new_mux
	auto pos_ = std::find(branch_cond->CntrlSuccs->begin(), branch_cond->CntrlSuccs->end(), branch);
	assert(pos_ != branch_cond->CntrlSuccs->end());
	int idx_ = pos_ - branch_cond->CntrlSuccs->begin();
	branch_cond->CntrlSuccs->at(idx_) = new_mux;

	// in0 is the new condition because we need to look at it only when the other condition is false
	new_mux->CntrlPreds->push_back(new_extra_cond);
	new_extra_cond->CntrlSuccs->push_back(new_mux);


	// in1 is constant 1
	ENode* cst_condition = new ENode(Cst_, std::to_string(1).c_str(), target_bb);
	...
	new_mux->CntrlPreds->push_back(cst_condition);
	cst_condition->CntrlSuccs->push_back(new_mux);

	// trigger this constant from SOurce
	ENode* source_node = new ENode(Source_, "source", target_bb);
	source_node->id    = source_id++;
	cst_condition->JustCntrlPreds->push_back(source_node);
	source_node->JustCntrlSuccs->push_back(cst_condition);
	enode_dag->push_back(source_node);

	new_mux->CntrlSuccs->push_back(branch);
	// let the new_mux replace the old condition of branch in the branch preds
	auto pos = std::find(branch->CntrlPreds->begin(), branch->CntrlPreds->end(), branch_cond);
	assert(pos != branch->CntrlPreds->end());
	int idx = pos - branch->CntrlPreds->begin();
	branch->CntrlPreds->at(idx) = new_mux;

	// TODO: DOUBLE CHECK THE NEGATION SIGNS HERE!!!!
	new_mux->is_advanced_component = true;
	new_mux->is_negated_input->push_back(false);
	new_mux->is_negated_input->push_back(is_negated_new_extra_cond);
	new_mux->is_negated_input->push_back(false);
...
void CircuitGenerator::addSuppress_with_loops(networkType network_flag) {
	std::ofstream newest_dbg_file;
	if(network_flag == data)
		newest_dbg_file.open("newest_check_loop_branches.txt");
    ...
    switch(network_flag) {
    	case data:
    		file_name = "check_Shannon's_on_Suppress_conds_data.txt";
    	break;
    	case constCntrl:
    		file_name = "check_Shannon's_on_Suppress_conds_constants.txt";
    	break;
    	case memDeps:
    		file_name = "check_Shannon's_on_Suppress_conds_LSQ_mem.txt";
    }
    std::ofstream dbg_file_6;
    dbg_file_6.open(file_name.c_str());


    switch(network_flag) {
    	case data:
    		file_name = "check_calculate_f_supp_data.txt";
    	break;
    	case constCntrl:
    		file_name = "check_calculate_f_supp_constants.txt";
    	break;
    	case memDeps:
    		file_name = "check_calculate_f_supp_LSQ_mem.txt";
    }
	std::ofstream dbg_file_7;
    dbg_file_7.open(file_name.c_str());
	...
	for(auto& enode : *enode_dag) {  
		...
		for (int h = 0; h < cons_preds->size(); h++) {  // loop over its producers
			...
			if(more_prod_than_cons) {
				...
				// AYA: 19/06/2023: commented out the following and replaced it with the code that inserts a Branch at every loop exit!!!
				...
				// OLd debugs
				/*if(consumer->type == Inst_) {
					if(consumer->Instr->getOpcode() == Instruction::Store) {
						for(int kkk = 0; kkk < loops_in_nest.size(); kkk++) {
							newest_dbg_file << BBMap->at(loops_in_nest.at(kkk)->getHeader())->Idx + 1 << "\n";

							if(BBMap->at(consumer->BB)->loop == loops_in_nest.at(kkk))
								newest_dbg_file << "\t The previous is that of the consumer!!\n";

							Loop* temp_loop = BBMap->at(consumer->BB)->loop;
							while(temp_loop!=nullptr) {
								if(temp_loop == loops_in_nest.at(kkk)) {
									// erase this loop
									//loops_in_nest.erase(loops_in_nest.begin() + uu);
									newest_dbg_file << "\t \t The previous is a parent loop of the consumer!!\n";
								}
								temp_loop = temp_loop->getParentLoop();
							}
						}
					}
				}
				...
				////// AYA: 19/09/2023: I brought back this snippet instead of the one commented above because the un_tagger requires that each loop in a loop-nest has its own branch for the effectiveness of its operation
						// BUt, I realized that the above function returns all loops that the producer is inside even those that the consumer is insde as well
							// So, I'm adding the following extra code to eliminate loops that the consumer is inside to avoid adding wrong branches!!
				for(int uu = 0; uu < loops_in_nest.size(); uu++) {
					if(consumer->BB == nullptr)
						break;
					/*if(BBMap->at(consumer->BB)->loop == loops_in_nest.at(uu)) {
						// erase this loop
						loops_in_nest.erase(loops_in_nest.begin() + uu);
					}*/ //else {
						// also erase if it is a parent of the loop of the consumer 
						Loop* temp_loop = BBMap->at(consumer->BB)->loop;

						while(temp_loop!=nullptr) {
							if(temp_loop == loops_in_nest.at(uu)) {
								// erase this loop
								loops_in_nest.erase(loops_in_nest.begin() + uu);
							}
							temp_loop = temp_loop->getParentLoop();
				...
				for(int i = 0; i < loops_in_nest.size(); i++) {
					...
					ENode* new_branch = insertBranch(network_flag, loop_exit_condition_enode, my_producer, consumer, is_negated_flag);
				...
				if(producer == consumer) {
					...
					// AYAA 16/12/2022: Currently putting all the networks in this condition, because at the moment, I support no REGENERATE in any network!
					if(network_flag == constCntrl || network_flag == memDeps || network_flag == data) {
						...
						// AYAA 09/11/2022: NEW CODE REPLACING THE ABOVE! Currently testing it on the data network only!!
						...
						dbg_file_2 << "\n###################################\n";
						dbg_file_2 << "Right before calling constructSOP, the content of modif_consBB_deps is: ";
						for(int u = 0; u < modif_consBB_deps.size(); u++) {
							dbg_file_2 << "BB" << modif_consBB_deps.at(u) + 1 << ", ";
						}
						dbg_file_2 << "\n";
						constructSOP(dbg_file_2, BBMap->at(producer->BB), consumerBB, modif_consBB_deps, f_cons, true);  // the last parameter represents an experiment_flag that informs the function that I'm coming frm a call towards the REGENERATE component
			...
			if(!llvm_phi_case) {
				dbg_file_6 << "\n#############################################\n";
				dbg_file_6 << "Right before calling calculate_F_supp\n";
				dbg_file_6 << "The f_cons is (not considering negations!!): ";
				for(int i = 0; i < f_cons.size(); i++) {
					for(int j = 0; j < f_cons.at(i).size(); j++) {
						dbg_file_6 << "C" << f_cons.at(i).at(j).pierBB_index + 1 << " & ";
					}
					dbg_file_6 << " + ";
				}
				dbg_file_6 <<"\n";
				dbg_file_6 << "The f_prod is (not considering negations!!): ";
				for(int i = 0; i < f_prod.size(); i++) {
					for(int j = 0; j < f_prod.at(i).size(); j++) {
						dbg_file_6 << "C" << f_prod.at(i).at(j).pierBB_index + 1 << " & ";
					}
					dbg_file_6 << " + ";
				}
				dbg_file_6 <<"\n";
				...
				dbg_file_6 << "\n#############################################\n";
				dbg_file_6 << "Right after returning from calculate_F_supp\n";
				dbg_file_6 << "The f_supp is (not considering negations!!): ";
				for(int i = 0; i < f_supp.size(); i++) {
					for(int j = 0; j < f_supp.at(i).size(); j++) {
						dbg_file_6 << "C" << f_supp.at(i).at(j).pierBB_index + 1 << " & ";
					}
					dbg_file_6 << " + ";
				}
				dbg_file_6 <<"\n";
...
/**
 * @brief check if the hacky Merge for the initialization token is already present in the BB
 * @param 
 */
...
ENode* CircuitGenerator::applyShannon(std::ofstream& dbg_file, std::ofstream& dbg_file_5, BasicBlock* target_bb, const std::vector<std::vector<pierCondition>>& f, bool& is_condition_negated, bool print_flag) {
	...
	dbg_file << "\n#############################################\n";
	dbg_file << "Inside applyShannon function\n";
	dbg_file << "The function before converting it to a string is (not considering negations!!): ";
	for(int i = 0; i < f.size(); i++) {
		for(int j = 0; j < f.at(i).size(); j++) {
			dbg_file << "C" << f.at(i).at(j).pierBB_index + 1 << " & ";
		}
		dbg_file << " + ";
	}
	dbg_file <<"\n";
...
void CircuitGenerator::new_get_binary_string_minterms_Shannons(std::ofstream& dbg_file, const std::vector<std::vector<pierCondition>>& funct, const std::vector<int>& condition_variables, std::vector<std::string>& minterms_only_in_binary, bool print_flag) {
	// SOme debug info.
	dbg_file << "\n#############################################\n";
	dbg_file << "Inside new_get_binary_string_minterms_Shannons function\n";
	dbg_file << "The function before converting it to a string is (not considering negations!!): ";
	for(int i = 0; i < funct.size(); i++) {
		for(int j = 0; j < funct.at(i).size(); j++) {
			dbg_file << "C" << funct.at(i).at(j).pierBB_index + 1 << " & ";
		}
		dbg_file << " + ";
	}
	dbg_file <<"\n";
...
*/
		...
		for(int i = 0; i < modif_BB_deps.size(); i++) {
			...
			if(pos != final_paths->at(kk).end()) {
				...
				if(startBB == bbnode) {
					...
					// Aya: 06/12/2022: the following code is probably wrong and old and will not be needed!!
	...
	// 12/11/2022: with the smart way of REGENERATing tokens in cases where the consumer is inside one or more loops that the producer is not inside, 
				// the outer loops enclosing the consumer in a loop nest will not be found in any path because the producer and consumer are in the same BB (i.e., because they are both the Phi of our insertion)
	...
	for(int i = 0; i < modif_BB_deps.size(); i++) {
		...
		if(!found && experiment_flag) {   // AYA: IN some cases, we could have some modiBB_deps not in any path but for different reasons
	...
	// 06/12/2022: not sure if this statement still applies
