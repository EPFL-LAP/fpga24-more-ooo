...
void CircuitGenerator::Fix_my_PhiPreds(LoopInfo& LI, networkType network_flag) {
	...
	*/
	...
	for(auto & enode :*enode_dag) {
		// AYA: 15/09/2022: conditions related to stuff added for the smart interfacing with the LSQ!!!
		if(enode->type == Phi_c && enode->is_mem_dep_component) {
			continue;   // skip if you come across a Phi_c that was added for managing GSA in the smart LSQ interfacing!! 
		}

		if(enode->type == Phi_n && enode->is_mux_for_cond) {
			continue; // Or if you come across the Merge that feeds it!!
		}
	...
	for(auto & enode :*enode_dag) {
		...
		if(enode->type == phi_type && !enode->is_redunCntrlNet) {
			...
			} else {  // AYA: 14/09/2022: added this to account for the opposite case (i.e., a Phi_c having preds in the CntrlOrder, so we should not expect preds from it in the JustCntrl!!)
...
void CircuitGenerator::checkPhiModify(Loop* cons_loop, BasicBlock* producer_BB, BasicBlock* consumer_BB, ENode* orig_producer_enode, ENode* orig_consumer_enode, networkType network_flag, std::ofstream& h_file, std::vector<ENode*>* new_phis) {
	...
	while(cons_loop != nullptr) {	
		if(!cons_loop->contains(producer_BB)) {
        	...
        	// AYA: 30/10/2022: added a condition to not skip phi_c that is_non_loop_gsa_mux (added in smart LSQs!)
			if(loop_header != consumer_BB || (orig_consumer_enode->type != Phi_ && orig_consumer_enode->type != Phi_n && orig_consumer_enode->type != Phi_c ) || (orig_consumer_enode->type == Phi_c && orig_consumer_enode->is_non_loop_gsa_mux) ){
...
void CircuitGenerator::removeExtraPhisWrapper(networkType network_flag) {
	std::ofstream extra_phis_dbg_file;
	extra_phis_dbg_file.open("INNER_remove_extra_phis_dbg.txt");
	extra_phis_dbg_file << "\n********************************************************\n ";
	extra_phis_dbg_file << "\nBefore removeExtraPhis\n ";
...
void CircuitGenerator::deleteExtraPhis(networkType network_flag) {
	...
	for(int i = 0; i < enode_dag->size(); i++) {
		// AYA: 15/09/2022: conditions related to stuff added for the smart interfacing with the LSQ!!!
		if(enode_dag->at(i)->type == Phi_c && enode_dag->at(i)->is_mem_dep_component) {
			continue;   // skip if you come across a Phi_c that was added for managing GSA in the smart LSQ interfacing!! 
		}

		if(enode_dag->at(i)->type == Phi_n && enode_dag->at(i)->is_mux_for_cond) {
			continue; // Or if you come across the Merge that feeds it!!
		}
...
void CircuitGenerator::removeExtraPhis(networkType network_flag) {
	...
	for(int i = 0; i < enode_dag_cpy->size(); i++) {
		// AYA: 15/09/2022: conditions related to stuff added for the smart interfacing with the LSQ!!!
		if(enode_dag_cpy->at(i)->type == Phi_c && enode_dag_cpy->at(i)->is_mem_dep_component) {
			continue;   // skip if you come across a Phi_c that was added for managing GSA in the smart LSQ interfacing!! 
		}

		if(enode_dag_cpy->at(i)->type == Phi_n && enode_dag_cpy->at(i)->is_mux_for_cond) {
			continue; // Or if you come across the Merge that feeds it!!
		}
		...
		// check if this enode_1 is of type phi
		if(enode_1->type == phi_type && !enode_1->is_redunCntrlNet) {
			assert(enode_1->producer_to_branch != nullptr);
			...
			while(j < enode_dag_cpy->size()) {
				// AYA: 15/09/2022: conditions related to stuff added for the smart interfacing with the LSQ!!!
				if(enode_dag_cpy->at(j)->type == Phi_c && enode_dag_cpy->at(j)->is_mem_dep_component) {
					j++;  // increment the counter before skipping the iteration
					continue;   // skip if you come across a Phi_c that was added for managing GSA in the smart LSQ interfacing!! 
				}

				if(enode_dag_cpy->at(j)->type == Phi_n && enode_dag_cpy->at(j)->is_mux_for_cond) {
					j++;   // increment the counter before skipping the iteration
					continue; // Or if you come across the Merge that feeds it!!
				}
...
void CircuitGenerator::checkLoops(Function& F, LoopInfo& LI, networkType network_flag) {
    ...
    for (auto& enode : *enode_dag) {
		...
		// Aya: 19/10/2022 added a condition to skip INIT components (i.e., phi_n merges)(those are mainly added in the smart LSQ because the rest are added later in addSuppress)
...
// TAKE CARE: URGENT!!
// Aya: it is temporary and it only supports the CntrlPreds,CntrlSuccs data networks.. Need to extend it to the other networks as well!
void CircuitGenerator::TEMP_deleteExtraPhis(networkType network_flag) {
	node_type phi_type;
	...
	// loop over the enodes in search for a Phi of your insertion that is not is_non_loop_gsa_mux and is not is_mem_dep_component
	for(auto& enode: *enode_dag) {
		bool constant_control_network_flag = false;

		if(enode->is_non_loop_gsa_mux || enode->is_mem_dep_component) 
			continue;  // skip any nodes that were inserted for managing the smart network of the LSQs
		if(enode->type == phi_type) {
			...
			switch(network_flag) {
				...
				case memDeps:
					if(enode->JustCntrlPreds->size() > 0) {
						assert(enode->CntrlOrderPreds->size() == 0 && enode->CntrlOrderSuccs->size() == 0);
						assert(enode->JustCntrlSuccs->size() > 0);
						enode_succs = enode->JustCntrlSuccs;
						enode_preds = enode->JustCntrlPreds;

						constant_control_network_flag = true;
						...
						constant_control_network_flag = false;
					}
			}

			assert(!enode->is_non_loop_gsa_mux && !enode->is_mem_dep_component); 
			// My goal is to search for a Phi_n that has a non-Phi succ (excluding that it will always have itself as the successor, but I'm speaking of the second successor!) and a Phi as its producer (again excluding itself!)
			assert(enode_preds->size() > 0 && enode_succs->size() > 0);
			assert(enode_preds->at(0) != enode && enode_succs->at(0) != enode);

			if(enode_preds->at(0)->type == phi_type && enode_succs->at(0)->type != phi_type) {  // identify the last Phi_n in the chain of phis (i.e., the one closest to the consumer!)
				ENode* current_node = enode;

				switch(network_flag) {
					case data:
						while(/*current_node != enode->producer_to_branch && */current_node->CntrlPreds->at(0) != enode->producer_to_branch) {  // keep looping over all PHi_n in the chain until you arrive to the producer!
							// I need to replace the enode->CntrlPreds->at(0) with the pred of the pred and delete the enode->CntrlPreds->at(0) from the enode_dag!
							// search for enode->CntrlPreds->at(0) in the succs of enode->producer_to_branch
							ENode* pred_of_pred = current_node->CntrlPreds->at(0)->CntrlPreds->at(0);  
							ENode* pred = current_node->CntrlPreds->at(0);
							auto pos = std::find((pred_of_pred)->CntrlSuccs->begin(), (pred_of_pred)->CntrlSuccs->end(), pred);
							assert(pos != (pred_of_pred)->CntrlSuccs->end());
							int idx = pos - (pred_of_pred)->CntrlSuccs->begin();
							(pred_of_pred)->CntrlSuccs->at(idx) = current_node;
							
							// search for the enode->CntrlPreds->at(0) in the enode_dag
							auto pos_enode_dag = std::find(enode_dag->begin(), enode_dag->end(), pred);
							enode_dag->erase(pos_enode_dag);

							// overwrite the node
							current_node->CntrlPreds->at(0) = pred_of_pred;

							//current_node = pred_of_pred;//current_node->CntrlPreds->at(0);   
						}
					break;

					case constCntrl:
					case memDeps:
						if(constant_control_network_flag) {
							assert(current_node->JustCntrlPreds->size() > 0 && current_node->CntrlOrderPreds->size() == 0);
							while(/*current_node != enode->producer_to_branch && */current_node->JustCntrlPreds->at(0) != enode->producer_to_branch) {  // keep looping over all PHi_n in the chain until you arrive to the producer!
								// I need to replace the enode->CntrlPreds->at(0) with the pred of the pred and delete the enode->CntrlPreds->at(0) from the enode_dag!
								// search for enode->CntrlPreds->at(0) in the succs of enode->producer_to_branch
								ENode* pred_of_pred = current_node->JustCntrlPreds->at(0)->JustCntrlPreds->at(0);  
								ENode* pred = current_node->JustCntrlPreds->at(0);
								auto pos = std::find((pred_of_pred)->JustCntrlSuccs->begin(), (pred_of_pred)->JustCntrlSuccs->end(), pred);
								assert(pos != (pred_of_pred)->JustCntrlSuccs->end());
								int idx = pos - (pred_of_pred)->JustCntrlSuccs->begin();
								(pred_of_pred)->JustCntrlSuccs->at(idx) = current_node;
								...
								// overwrite the node
								current_node->JustCntrlPreds->at(0) = pred_of_pred;

								//current_node = pred_of_pred;//current_node->CntrlPreds->at(0);   
							}
						} else {
							assert(current_node->CntrlOrderPreds->size() > 0 && current_node->JustCntrlPreds->size() == 0);
							while(/*current_node != enode->producer_to_branch && */current_node->CntrlOrderPreds->at(0) != enode->producer_to_branch) {  // keep looping over all PHi_n in the chain until you arrive to the producer!
								// I need to replace the enode->CntrlPreds->at(0) with the pred of the pred and delete the enode->CntrlPreds->at(0) from the enode_dag!
								// search for enode->CntrlPreds->at(0) in the succs of enode->producer_to_branch
								ENode* pred_of_pred = current_node->CntrlOrderPreds->at(0)->CntrlOrderPreds->at(0);  
								ENode* pred = current_node->CntrlOrderPreds->at(0);
								auto pos = std::find((pred_of_pred)->CntrlOrderSuccs->begin(), (pred_of_pred)->CntrlOrderSuccs->end(), pred);
								assert(pos != (pred_of_pred)->CntrlOrderSuccs->end());
								int idx = pos - (pred_of_pred)->CntrlOrderSuccs->begin();
								(pred_of_pred)->CntrlOrderSuccs->at(idx) = current_node;
								...
								// overwrite the node
								current_node->CntrlOrderPreds->at(0) = pred_of_pred;
				...
				// OLD CODE
				// for now, I'm restricting it to the simple case of a max. of 2 loops in the nest!
				//assert(enode->CntrlPreds->at(0)->CntrlPreds->at(0)->type != Phi_n && enode->CntrlPreds->at(0)->CntrlPreds->at(0) == enode->producer_to_branch);
				// I need to replace the enode->CntrlPreds->at(0) with the pred of the pred and delete the enode->CntrlPreds->at(0) from the enode_dag!
				// search for enode->CntrlPreds->at(0) in the succs of enode->producer_to_branch
				/*auto pos = std::find((enode->producer_to_branch)->CntrlSuccs->begin(), (enode->producer_to_branch)->CntrlSuccs->end(), enode->CntrlPreds->at(0));
				assert(pos != (enode->producer_to_branch)->CntrlSuccs->end());
				int idx = pos - (enode->producer_to_branch)->CntrlSuccs->begin();
				(enode->producer_to_branch)->CntrlSuccs->at(idx) = enode;
				
				// search for the enode->CntrlPreds->at(0) in the enode_dag
				auto pos_enode_dag = std::find(enode_dag->begin(), enode_dag->end(), enode->CntrlPreds->at(0));
				enode_dag->erase(pos_enode_dag);

				// overwrite the node
				enode->CntrlPreds->at(0) = enode->producer_to_branch; */
