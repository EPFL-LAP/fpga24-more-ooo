...
void CircuitGenerator::fix_INIT_inputs() {
	...
	for(auto& enode :*enode_dag) {
		if(enode->type != Phi_n)
			continue;   // skip the non INIT

		if(enode->is_init_token_merge) {
			assert(!enode->isMux);
			// loop over its preds
			for(int i = 0; i < enode->CntrlPreds->size(); i++) {
				if(enode->CntrlPreds->at(i)->type == Cst_) {
					enode->CntrlPreds->at(i)->cstValue = 0;
				} 
			}

			assert(enode->is_advanced_component);
			assert(enode->is_negated_input->size() == 2);
			// make sure that both inputs are not negated because if it is negated from above, this means it is against my current convention
			enode->is_negated_input->at(0) = false;
			enode->is_negated_input->at(1) = false;

			init = enode;

			// find all Muxes connected to this INIT (i.e., Muxes in the same BB as the INIT) and make sure to flip the inputs of any of such Mux to meet our new convention
			assert(init != nullptr);
			for(auto& enode_2 : *enode_dag) {
				if(enode_2->isMux && enode_2->BB == init->BB) {
					if(enode_2->CntrlPreds->at(0) == init || enode_2->CntrlPreds->at(0)->CntrlPreds->at(0) == init){
						if(enode_2->type == Phi_ || enode_2->type == Phi_n) {
							ENode* temp = enode_2->CntrlPreds->at(1);
							enode_2->CntrlPreds->at(1) =  enode_2->CntrlPreds->at(2);
							enode_2->CntrlPreds->at(2) = temp;
						} else {
							assert(enode_2->type == Phi_c);
							ENode* temp = enode_2->JustCntrlPreds->at(0);
							enode_2->JustCntrlPreds->at(0) =  enode_2->JustCntrlPreds->at(1);
							enode_2->JustCntrlPreds->at(1) = temp;
						}
					}
			...
			fix_start_generation(init);
		}
	}
}

void CircuitGenerator::fix_start_generation(ENode* init) {
	// reposition the constant to be in the loop header of the loop that is right above the loop header of the init (BB0 if the init is not in an inner loop)
			// and search for any constant in the same BB to feed your constant with its predecessor for correct token delivery from Start_

	// 1st: identify the BB to insert the Const in
	BasicBlock* target_const_bb = nullptr;
	Loop* target_loop = BBMap->at(init->BB)->loop;
	
	if(target_loop != nullptr)
		target_loop = target_loop->getParentLoop();

	if(target_loop == nullptr)
		return; // we do not need to move the const of the INIT because the INIT is not in an inner loop

	// search for the loop header BB of the target_loop
	for(auto& bbnode : *bbnode_dag) {
		if(bbnode->loop == target_loop && bbnode->is_loop_header) {
			target_const_bb = bbnode->BB;
			break;
		}
	}
	assert(target_const_bb != nullptr);

	std::string cst_name = std::to_string(1);

	for(int i = 0; i < init->CntrlPreds->size(); i++) {
		if(init->CntrlPreds->at(i)->type == Cst_) {
			ENode* new_const = new ENode(Cst_, cst_name.c_str(), target_const_bb);
			new_const->cstValue = 0;
			new_const->id = cst_id++;
			enode_dag->push_back(new_const);

			// cut the connection between this constant and the INIT and insert a new constant in the correct BB
			auto pos = std::find(init->CntrlPreds->at(i)->CntrlSuccs->begin(), init->CntrlPreds->at(i)->CntrlSuccs->end(), init);
			assert(pos != init->CntrlPreds->at(i)->CntrlSuccs->end());
			// erase the init from the succ of the pred
			init->CntrlPreds->at(i)->CntrlSuccs->erase(pos);

			// AYA: 27/09/2023: the following logic is incomplete since it fails (a) if the constant it hits is triggered by SOurce, (b) if the loop is not guaranteed to execute
				// therefore, I added some extra conditions that I labeled with today's date

			// search for a constant in the same BB to connect new_const to its preds
			for(int k = 0; k < enode_dag->size(); k++) {
				if(enode_dag->at(k)->type == Cst_ && enode_dag->at(k)->BB == target_const_bb
							&& !enode_dag->at(k)->is_const_source_triggered) {  // AYA: 27/09/2023: added the extra AND
					ENode* similar_const = enode_dag->at(k);
					assert(similar_const->JustCntrlPreds->size() == 1);

					// 11/09/2023: commenting this to have the START port of INIT dataless and connected directly to START
					new_const->JustCntrlPreds->push_back(similar_const->JustCntrlPreds->at(0));
					similar_const->JustCntrlPreds->at(0)->JustCntrlSuccs->push_back(new_const);
					// 11/09/2023: replaced the previous lines with the following
					/*init->CntrlPreds->at(i) = similar_const->JustCntrlPreds->at(0);
					similar_const->JustCntrlPreds->at(0)->JustCntrlSuccs->push_back(init);*/
					

					// commented the following because now I call this function before adding forks
					/*if(similar_const->JustCntrlPreds->at(0)->type == Fork_c) {
						// connect the new_const to this existing fork
						new_const->JustCntrlPreds->push_back(similar_const->JustCntrlPreds->at(0));
						similar_const->JustCntrlPreds->at(0)->JustCntrlSuccs->push_back(new_const);
					} else {
						// insert a new Fork
						ENode* new_fork = new ENode(Fork_c, "forkC", target_const_bb);
						new_fork->id = fork_id++;
						enode_dag->push_back(new_fork);

						// the pred of the fork should be enode_dag->at(k)->JustCntrlPreds->at(0)
						new_fork->JustCntrlPreds->push_back(similar_const->JustCntrlPreds->at(0));
						assert(similar_const->JustCntrlPreds->at(0)->JustCntrlSuccs->size() == 1);
						similar_const->JustCntrlPreds->at(0)->JustCntrlSuccs->at(0) = new_fork;

						new_fork->JustCntrlSuccs->push_back(similar_const);
						similar_const->JustCntrlPreds->at(0) = new_fork;

						new_fork->JustCntrlSuccs->push_back(new_const);
						new_const->JustCntrlPreds->push_back(new_fork);
					}*/
					break;
				}
			}

			// AYA: 27/09/2023: further to the comment above, the constant token should not be delivered to the INIT if the loop does not execute,
				// SO, if the BB of the INIT does not post-dominate the constant, insert a Branch with the right condition to discard the constant
						// TODO: for now, the Branch and its condition are assumed to be very simplistic, but this should be changed to be more general by applying the algorithm of FPL'22 
			if(!my_post_dom_tree->properlyDominates(init->BB, target_const_bb)) {
				// insert a Branch with the condition from target_const_bb between the new_const and the INIT
				ENode* branch = new ENode(Branch_n, "branch", target_const_bb);  // we insert the Branch in the producer BB 
				branch->id = branch_id++;
				enode_dag->push_back(branch);

				branch->producer_to_branch = new_const;
				...
				branch->CntrlPreds->push_back(new_const);
				new_const->CntrlSuccs->push_back(branch);

				///////////////////////// connecting the INIT to one of the Succs of the Branch
				...
				if(my_post_dom_tree->properlyDominates(init->BB, falseBB) || init->BB == falseBB) 	{
					// as a sanity check, it shouldn't post-dom the trueBB!!
					assert(!my_post_dom_tree->properlyDominates(init->BB, trueBB) || init->BB == trueBB);
					branch->false_branch_succs->push_back(init);
				} else {
					assert(my_post_dom_tree->properlyDominates(init->BB, trueBB) || init->BB == trueBB);
					branch->true_branch_succs->push_back(init);
				}

				// connect the condition of the Branch
				...
				// connect the init to the succs of the Branch and the Branch to the preds of the INIT
				// replace this pred of init with new_const
				init->CntrlPreds->at(i) = branch;

			} else {
				// directly connect the new_const and INIT
				// 11/09/2023: comment this if we need to have the START port of INIT dataless and connected directly to START
				// replace this pred of init with new_const
				init->CntrlPreds->at(i) = new_const;
				new_const->CntrlSuccs->push_back(init);
			}
	...
	// loop over all constants to delete any constant that does not have any left succs
	for(int j = 0; j < enode_dag->size(); j++) {
		if(enode_dag->at(j)->type == Cst_ && enode_dag->at(j)->JustCntrlSuccs->size() + enode_dag->at(j)->CntrlSuccs->size() + enode_dag->at(j)->CntrlOrderSuccs->size() == 0) {
			// first remove this enode from the succs of its pred
			ENode* const_to_delete = enode_dag->at(j);
			auto pos = std::find(const_to_delete->JustCntrlPreds->at(0)->JustCntrlSuccs->begin(), const_to_delete->JustCntrlPreds->at(0)->JustCntrlSuccs->end(), const_to_delete);
			assert(pos != const_to_delete->JustCntrlPreds->at(0)->JustCntrlSuccs->end());
			const_to_delete->JustCntrlPreds->at(0)->JustCntrlSuccs->erase(pos);

			enode_dag->erase(j + enode_dag->begin());
		}
	}
}

// AYA: 06/07/2023: added the following function to adjust the connections between ST, MC and END
// this function is called in the main after addExitC so it assumes that the MC is already connected to the END_node
void CircuitGenerator::fix_ST_MC_interfaces(bool fix_mc_st_interfaces_flag_yes_extra_ST_output) {
	ENode* mc_node = nullptr;

	ENode* end_node = nullptr;
	int end_node_idx_in_mc_succ = -1;
	int mc_node_idx_in_end_pred = -1;

	ENode* store_node = nullptr;

	// loop over all enodes searching for an MC
	for(int i = 0; i < enode_dag->size(); i++) {
		if(enode_dag->at(i)->type == MC_) {
			mc_node = enode_dag->at(i);

			// DEBUGGING
			assert(mc_node->CntrlOrderSuccs->size() == 1);

			// identify the index connecting it to END_ (in the CntrlOrder network because this is where we connect to END_ in addExitC )
			for(int j = 0; j < mc_node->CntrlOrderSuccs->size(); j++) {
				if(mc_node->CntrlOrderSuccs->at(j)->type == End_) {
					end_node = mc_node->CntrlOrderSuccs->at(j);
					end_node_idx_in_mc_succ = j;

					auto pos = std::find(end_node->CntrlOrderPreds->begin(), end_node->CntrlOrderPreds->end(), mc_node);
					assert(pos != end_node->CntrlOrderPreds->end());
					mc_node_idx_in_end_pred = pos - end_node->CntrlOrderPreds->begin();

					// New: 18/07/2023: I decided to completely erase the connection between the MC and END
					end_node->CntrlOrderPreds->erase(pos);
					mc_node->CntrlOrderSuccs->erase(j + mc_node->CntrlOrderSuccs->begin());
					///////////////////////////////////////////////////////////////////////////////////////

					break; // once you find END, you are done because you do not need to search for anything else
				}
			}


			// loop over all ST connected to the current MC that we are processing (in the CntrlPreds and CntrlSuccs network since this is where they are conencted in Memory.cpp )
			for(int j = 0; j < mc_node->CntrlPreds->size(); j++) {
				if(mc_node->CntrlPreds->at(j)->type == Inst_) {
					assert(mc_node->CntrlPreds->at(j)->Instr != nullptr);

					if(isa<StoreInst>(mc_node->CntrlPreds->at(j)->Instr)) {
						store_node = mc_node->CntrlPreds->at(j);

						assert(end_node != nullptr && end_node_idx_in_mc_succ != -1 && mc_node_idx_in_end_pred != -1);
						// Replace the MC -> END connection with MC -> ST and ST -> END
						// ST -> END

						if(fix_mc_st_interfaces_flag_yes_extra_ST_output) {
							//end_node->CntrlOrderPreds->at(mc_node_idx_in_end_pred) = store_node;
							end_node->CntrlOrderPreds->push_back(store_node);  // for now, I will keep the connection between MC and END
							store_node->CntrlOrderSuccs->push_back(end_node);
						}

						// MC -> ST
						//if(fix_mc_st_interfaces_flag_yes_extra_ST_output) 
							//mc_node->CntrlOrderSuccs->at(end_node_idx_in_mc_succ) = store_node;

						store_node->CntrlOrderPreds->push_back(mc_node);

						//if(!fix_mc_st_interfaces_flag_yes_extra_ST_output)
							mc_node->CntrlOrderSuccs->push_back(store_node);  // TEMPORARY LINE AND SHOULD UNCOMMENT THE ABOVE LINES WHEN DONE EXPERIMENTING!!
					}
				}
			}
			remove_MC_ST_counter(mc_node);

		}
	}

	// THe problem now is that my code assumes that there is a single ST because it replaces the single connection between MC and END with this ST
		// BUT, if there are multiple ST, this replacement should happen once and the rest should be just push_backs
			// BUT, the main challenge is that the MC used to have a single output to END. Now, I'm expecting it to have many outputs (one for each ST)
				// and I'm still not sure how to do this!

	// TODO!!!! NEW PROBLEM!! MC has a single port for taking ST requests. I do not know how to have a variable number of ports there
			// TODO: check how this is managed with the data and address ports when there are many ST to a single MC.
				// For now, assume that we can only support a single ST to an MC!!  
}

void CircuitGenerator::remove_MC_ST_counter(ENode* mc_node) {
	ENode* const_node = nullptr;
	for(int i = 0; i < mc_node->CntrlOrderPreds->size(); i++) {
		if(mc_node->CntrlOrderPreds->at(i)->type == Cst_) {
			const_node = mc_node->CntrlOrderPreds->at(i);

			// cut the connection between this constant and START
			assert(const_node->JustCntrlPreds->size() == 1);
			//assert(const_node->JustCntrlPreds->at(0)->type == Start_);	// this is very strong because it can also be a FORK or a Branch

			auto pos = std::find(const_node->JustCntrlPreds->at(0)->JustCntrlSuccs->begin(), const_node->JustCntrlPreds->at(0)->JustCntrlSuccs->end(), const_node);
			assert(pos != const_node->JustCntrlPreds->at(0)->JustCntrlSuccs->end());
			const_node->JustCntrlPreds->at(0)->JustCntrlSuccs->erase(pos);

			if(const_node->JustCntrlPreds->at(0)->type == Branch_c) {
				// some sanity checks
				assert(const_node->JustCntrlPreds->at(0)->JustCntrlSuccs->size() == 1);

				// THis is an extreme assumption that I know might not be true in a general case, but for now I do this assert to let me know when the assumption is violates
				assert(const_node->JustCntrlPreds->at(0)->JustCntrlSuccs->at(0)->type == Sink_); 

				// delete the sink from the enode_dag
				auto pos_enode_dag_ = std::find(enode_dag->begin(), enode_dag->end(), const_node->JustCntrlPreds->at(0)->JustCntrlSuccs->at(0));
				assert(pos_enode_dag_ != enode_dag->end());
				enode_dag->erase(pos_enode_dag_);

				// delete the Branch_c from the enode_dag
				auto pos_enode_dag = std::find(enode_dag->begin(), enode_dag->end(), const_node->JustCntrlPreds->at(0));
				assert(pos_enode_dag != enode_dag->end());
				enode_dag->erase(pos_enode_dag);

				const_node->JustCntrlPreds->at(0)->JustCntrlSuccs->at(0)->JustCntrlPreds->clear();
				const_node->JustCntrlPreds->at(0)->JustCntrlSuccs->clear();


				// search for the Branch in the succs of its preds to delete it from there
				for(int i = 0; i < const_node->JustCntrlPreds->at(0)->JustCntrlPreds->size(); i++) {
					auto pos = std::find(const_node->JustCntrlPreds->at(0)->JustCntrlPreds->at(i)->JustCntrlSuccs->begin(), const_node->JustCntrlPreds->at(0)->JustCntrlPreds->at(i)->JustCntrlSuccs->end(), const_node->JustCntrlPreds->at(0));
					assert(pos != const_node->JustCntrlPreds->at(0)->JustCntrlPreds->at(i)->JustCntrlSuccs->end());
					const_node->JustCntrlPreds->at(0)->JustCntrlPreds->at(i)->JustCntrlSuccs->erase(pos);
				}

				for(int i = 0; i < const_node->JustCntrlPreds->at(0)->CntrlPreds->size(); i++) {
					auto pos = std::find(const_node->JustCntrlPreds->at(0)->CntrlPreds->at(i)->CntrlSuccs->begin(), const_node->JustCntrlPreds->at(0)->CntrlPreds->at(i)->CntrlSuccs->end(), const_node->JustCntrlPreds->at(0));
					assert(pos != const_node->JustCntrlPreds->at(0)->CntrlPreds->at(i)->CntrlSuccs->end());
					const_node->JustCntrlPreds->at(0)->CntrlPreds->at(i)->CntrlSuccs->erase(pos);
				}

				const_node->JustCntrlPreds->at(0)->JustCntrlPreds->clear();
				const_node->JustCntrlPreds->at(0)->CntrlPreds->clear();

			} else {
				// extra sanity check, it can not be Branch_n!
				assert(const_node->JustCntrlPreds->at(0)->type != Branch_n);
			}
			
			// cut the connection between MC and const
			mc_node->CntrlOrderPreds->erase(i + mc_node->CntrlOrderPreds->begin());

			// delete the constant from the enode_dag
			auto pos_enode_dag = std::find(enode_dag->begin(), enode_dag->end(), const_node);
			assert(pos_enode_dag != enode_dag->end());
			enode_dag->erase(pos_enode_dag);

			break; // once you found the const, stop looping
...
/* The following function loops over forks 
	 in particular to make sure there are no extra useless ones and to handle the case of START having new succs, etc.
*/
void CircuitGenerator::fixForks() {
	// 1st remove redundant FOrk_c
	/*for(auto& enode: *enode_dag) {
		if(enode->type != Fork_c && enode->type != Fork_) 
			continue;  // skip all nodes except the Fork

		if(enode->JustCntrlSuccs->size() + enode->CntrlOrderSuccs->size() + enode->CntrlSuccs->size() == 1) {  // there is no need for this fork so the goal is to delete it
			if(enode->CntrlSuccs->size() == 1) {
				//assert(1 != 1);
				// 	search for the fork in the succ of its pred with the fork's succ
				auto pos_ = std::find(enode->CntrlPreds->at(0)->CntrlSuccs->begin(), enode->CntrlPreds->at(0)->CntrlSuccs->end(), enode);
				assert(pos_ != enode->CntrlPreds->at(0)->CntrlSuccs->end());
				int idx_ = pos_ - enode->CntrlPreds->at(0)->CntrlSuccs->begin();

				enode->CntrlPreds->at(0)->CntrlSuccs->at(idx_) = enode->CntrlSuccs->at(0);

				// search for the fork in the preds of its succs
				...
				int idx = pos - enode->CntrlSuccs->at(0)->CntrlPreds->begin();

				enode->CntrlSuccs->at(0)->CntrlPreds->at(idx) = enode->CntrlPreds->at(0);  // replace the fork with its pred


			} else if(enode->JustCntrlSuccs->size() == 1) {
				//assert(2 != 2);
				// 	search for the fork in the succ of its pred with the fork's succ
				auto pos_ = std::find(enode->JustCntrlPreds->at(0)->JustCntrlSuccs->begin(), enode->JustCntrlPreds->at(0)->JustCntrlSuccs->end(), enode);
				assert(pos_ != enode->JustCntrlPreds->at(0)->JustCntrlSuccs->end());
				int idx_ = pos_ - enode->JustCntrlPreds->at(0)->JustCntrlSuccs->begin();

				enode->JustCntrlPreds->at(0)->JustCntrlSuccs->at(idx_) = enode->JustCntrlSuccs->at(0);
				...
				int idx = pos - enode->JustCntrlSuccs->at(0)->JustCntrlPreds->begin();

				enode->JustCntrlSuccs->at(0)->JustCntrlPreds->at(idx) = enode->JustCntrlPreds->at(0);  // replace the fork with its pred
				...
				// 	search for the fork in the succ of its pred with the fork's succ
				auto pos_ = std::find(enode->CntrlOrderPreds->at(0)->CntrlOrderSuccs->begin(), enode->CntrlOrderPreds->at(0)->CntrlOrderSuccs->end(), enode);
				assert(pos_ != enode->CntrlOrderPreds->at(0)->CntrlOrderSuccs->end());
				int idx_ = pos_ - enode->CntrlOrderPreds->at(0)->CntrlOrderSuccs->begin();

				enode->CntrlOrderPreds->at(0)->CntrlOrderSuccs->at(idx_) = enode->CntrlOrderSuccs->at(0);
				...
				int idx = pos - enode->CntrlOrderSuccs->at(0)->CntrlOrderPreds->begin();

				enode->CntrlOrderSuccs->at(0)->CntrlOrderPreds->at(idx) = enode->CntrlOrderPreds->at(0);  // replace the fork with its pred

			}

			// search for the enode in enode_dag and erase it
	...
	ENode* fork_from_start = nullptr;

	// 2nd fix the Succs of Start_ if needed
	for(auto& enode: *enode_dag) {
		if(enode->type != Start_)
			continue; // search for Start_ nodes so skip all others

		// extra sanity check: the Start_ can not have any successor in the data network
		assert(enode->CntrlSuccs->size() == 0);

		start_node = enode;
		assert(enode->type == Start_);
		fork_from_start = nullptr;
		bool found = false;

		bool ruined_flag_succ = false;


		if(start_node->JustCntrlSuccs->size() + start_node->CntrlOrderSuccs->size() > 1) {
			ruined_flag_succ = true;
			// one of it successors must be a Fork_c (never a Fork_)!
				// 1st check the JustCntrlSuccs network
			for(int i = 0; i < start_node->JustCntrlSuccs->size(); i++) {
				assert(start_node->JustCntrlSuccs->at(i)->type != Fork_);
				if(start_node->JustCntrlSuccs->at(i)->type == Fork_c) {
					fork_from_start = start_node->JustCntrlSuccs->at(i);
			...
			//assert(fork_from_start != nullptr && fork_from_start->type == Fork_c);
				// 2nd check the CntrlOrderSuccs network
			for(int i = 0; i < start_node->CntrlOrderSuccs->size(); i++) {
				assert(start_node->CntrlOrderSuccs->at(i)->type != Fork_);
				if(found)
					assert(start_node->CntrlOrderSuccs->at(i)->type != Fork_c);  // extra sanity checks
				else {
					if(start_node->CntrlOrderSuccs->at(i)->type == Fork_c) {
						fork_from_start = start_node->JustCntrlSuccs->at(i);
						found = true;
						break;
					}
			...
			assert(fork_from_start != nullptr  && fork_from_start->type == Fork_c);  // at this point, we are sure we have the fork that is fed from Start_

			// connect all other successors of Start_ to this Fork, then clear the successors of Start_ to have only this Fork

				// 1st start by the successors in the JustCntrlSuccs network
			for(int i = 0; i < start_node->JustCntrlSuccs->size(); i++) {
				if(start_node->JustCntrlSuccs->at(i)->type ==  Fork_c) 
					continue;  // skip the fork_c
				// search for the start_node in the pred of its succ to replace it with the fork node
				auto pos = std::find(start_node->JustCntrlSuccs->at(i)->JustCntrlPreds->begin(), start_node->JustCntrlSuccs->at(i)->JustCntrlPreds->end(), start_node);
				assert(pos != start_node->JustCntrlSuccs->at(i)->JustCntrlPreds->end());
				int idx = pos - start_node->JustCntrlSuccs->at(i)->JustCntrlPreds->begin();
				start_node->JustCntrlSuccs->at(i)->JustCntrlPreds->at(idx) = fork_from_start;

				fork_from_start->JustCntrlSuccs->push_back(start_node->JustCntrlSuccs->at(i));

			}

				// 2nd repeat the same for the successors in the CntrlOrderSuccs network
				// 1st start by the successors in the JustCntrlSuccs network
			for(int i = 0; i < start_node->CntrlOrderSuccs->size(); i++) {
				if(start_node->CntrlOrderSuccs->at(i)->type ==  Fork_c) 
					continue;  // skip the fork_c
				// search for the start_node in the pred of its succ to replace it with the fork node
				auto pos = std::find(start_node->CntrlOrderSuccs->at(i)->CntrlOrderPreds->begin(), start_node->CntrlOrderSuccs->at(i)->CntrlOrderPreds->end(), start_node);
				assert(pos != start_node->CntrlOrderSuccs->at(i)->CntrlOrderPreds->end());
				int idx = pos - start_node->CntrlOrderSuccs->at(i)->CntrlOrderPreds->begin();
				start_node->CntrlOrderSuccs->at(i)->CntrlOrderPreds->at(idx) = fork_from_start;

				fork_from_start->CntrlOrderSuccs->push_back(start_node->CntrlOrderSuccs->at(i));
		...
		if(ruined_flag_succ) {
			if(start_node->JustCntrlSuccs->size() > 0){
				start_node->CntrlOrderSuccs->clear();
				start_node->JustCntrlSuccs->clear();
				start_node->JustCntrlSuccs->push_back(fork_from_start);
			} else {
				assert(start_node->CntrlOrderSuccs->size() > 0);
				start_node->CntrlOrderSuccs->clear();
				start_node->JustCntrlSuccs->clear();
				start_node->CntrlOrderSuccs->push_back(fork_from_start);
			}
...
void CircuitGenerator::deliver_ACK_tokens_wrapper() {

	deliver_ACK_tokens_cond_st();
	deliver_ACK_tokens_loop_st();

}

// this function assumes that there is already an existing Branch with our desired condition
void CircuitGenerator::deliver_ACK_tokens_loop_st() {
	ENode* store_node = nullptr;
	ENode* end_node = nullptr;

	// loop over the enode_dag in search for a store instruction
	for(auto& enode : *enode_dag) {
		if(enode->type != Inst_) 
			continue; // skip non instructions
		if(!isa<StoreInst>(enode->Instr))
			continue;  // skip non store
		
		// at this point, we are sure that the enode is of type ST
		store_node = enode;

		// search for the end_node
		for(auto& enode_ : *enode_dag) {
			if(enode_->type == End_) {
				end_node = enode_;
		...
		// check if the store is inside a loop
		if(BBMap->at(store_node->BB)->loop != nullptr) {
			// create a new Branch node to discard all Store ACK of all iterations except the last iteration where it should be delivered to END
			ENode* new_branch = new ENode(Branch_c, "branchC", store_node->BB);
			new_branch->id = branch_id++;
			enode_dag->push_back(new_branch);
			new_branch->producer_to_branch = store_node;

			new_branch->is_st_ack_to_end = true;

			// the new branch will take the store_node as its data input
			new_branch->CntrlOrderPreds->push_back(store_node);
				// replace the end_node with the Branch in the succs of the store_node
			auto pos = std::find(store_node->CntrlOrderSuccs->begin(), store_node->CntrlOrderSuccs->end(), end_node );
			assert(pos != store_node->CntrlOrderSuccs->end());
			int idx = pos - store_node->CntrlOrderSuccs->begin();
			store_node->CntrlOrderSuccs->at(idx) = new_branch;

			// then we should insert a Branch that takes as a condition the loop exits
			// search for a Branch that takes as a data input a node in the same BB as that of the store_node and has one of its outputs completely outside the loop
			ENode* existing_branch = nullptr;
			int is_false_succ = -1;  // 0 if it is on the true side, 1 if it is on the false side
			for(auto& enode : *enode_dag) {
				bool flag_1 = false;
				bool flag_2 = false;
				if((enode->type == Branch_n || enode->type == Branch_c) && enode != new_branch ) {
					assert(enode->producer_to_branch != nullptr);
					if(BBMap->at(enode->producer_to_branch->BB)->loop == BBMap->at(store_node->BB)->loop) {
						if(enode->type == Branch_n) {
							assert(enode->CntrlSuccs->size() == 2);

							for(int r = 0; r < enode->CntrlSuccs->size(); r++) {
								if(enode->CntrlSuccs->at(r)->BB != nullptr) {
									// check if it is outside of any loop
									if(BBMap->at(enode->CntrlSuccs->at(r)->BB)->loop == nullptr) {
										if(r == 0){
											flag_1 = true;
											// the succ .at(0) is outside the loop, so END will be connected to the true side not the false side of new_branch
											is_false_succ = 0;
										} else {
											assert(r == 1);
											is_false_succ = 1;
									...
									// this succ belongs to no BB so it is either Sink_ or End_
									assert(enode->CntrlSuccs->at(r)->type == Sink_ || enode->CntrlSuccs->at(r)->type == End_); // sanity check
									switch(enode->CntrlSuccs->at(r)->type) {
										case Sink_:
										if(r == 0){
											assert(enode->CntrlSuccs->at(1)->type != Sink_);
											if(enode->CntrlSuccs->at(1)->BB != nullptr) {
												if(BBMap->at(enode->CntrlSuccs->at(1)->BB)->loop != nullptr) {
													assert(BBMap->at(enode->CntrlSuccs->at(1)->BB)->loop == BBMap->at(store_node->BB)->loop);
													is_false_succ = 0;
												} else {
													is_false_succ = 1;
												}
											...
											assert(r==1);
											assert(enode->CntrlSuccs->at(0)->type != Sink_);
											if(enode->CntrlSuccs->at(0)->BB != nullptr) {
												if(BBMap->at(enode->CntrlSuccs->at(0)->BB)->loop != nullptr) {
													assert(BBMap->at(enode->CntrlSuccs->at(0)->BB)->loop == BBMap->at(store_node->BB)->loop);
													is_false_succ = 1;
												} else {
													is_false_succ = 0;
												}
										...
										break;
										case End_:  // I know that this can't be connected on the CntrlSuccs data network but I'm adding it just in case
											if(r == 0)
												// the succ .at(0) is outside the loop, so END will be connected to the true side not the false side of new_branch
												is_false_succ = 0;
											else {
									...
									if(r == 0)
										flag_1 = true;  // in both cases, this is the candidate Branch that we desire to inherit the condition from
									else 
										flag_2 = true;
								}
							}
							...
							if(flag_1 || flag_2) {
								assert(is_false_succ != -1);

									// the condition of the new_branch should be the same as that branch
								existing_branch = enode;
								// manage the is_negated_inputs following this Branch
								if(existing_branch->is_advanced_component) {
									new_branch->is_advanced_component = true;
									assert(existing_branch->is_negated_input->size() == 2);
									for(int r = 0; r < existing_branch->is_negated_input->size(); r++)
										new_branch->is_negated_input->push_back(existing_branch->is_negated_input->at(r));
								} else {
									new_branch->is_advanced_component = false;
								}

								assert(existing_branch->type == Branch_n);
								// the condition is at CntrlPreds->at(1)
								if(existing_branch->CntrlPreds->at(1)->type == Fork_ || existing_branch->CntrlPreds->at(1)->type == Fork_c) {
									new_branch->CntrlPreds->push_back(existing_branch->CntrlPreds->at(1));
									existing_branch->CntrlPreds->at(1)->CntrlSuccs->push_back(new_branch);
								} else {
									assert(existing_branch->CntrlPreds->at(1)->CntrlSuccs->size() == 1); // should have exactly one succ
									assert(existing_branch->CntrlPreds->at(1)->CntrlSuccs->at(0) == existing_branch); // the single succ is simply the existing_branch
									// create a new Fork
									ENode* new_fork = new ENode(Fork_, "fork", store_node->BB);
									new_fork->id = fork_id++;
									enode_dag->push_back(new_fork);
									new_fork->CntrlPreds->push_back(existing_branch->CntrlPreds->at(1));
									existing_branch->CntrlPreds->at(1)->CntrlSuccs->at(0) = new_fork;

									new_fork->CntrlSuccs->push_back(new_branch);
									new_branch->CntrlPreds->push_back(new_fork);

									new_fork->CntrlSuccs->push_back(existing_branch);
									existing_branch->CntrlPreds->at(1) = new_fork;
								} 

								break;  // you found the desired branch so stop looping
							}
						} else {
							assert(enode->type == Branch_c);
							assert(enode->CntrlPreds->size() == 1);  // should contain only the condition 
							if(enode->JustCntrlSuccs->size() > 0) {
								for(int r = 0; r < enode->JustCntrlSuccs->size(); r++) {
									if(enode->JustCntrlSuccs->at(r)->BB != nullptr) {
										// check if it is outside of any loop
										if(BBMap->at(enode->JustCntrlSuccs->at(r)->BB)->loop == nullptr) {
											if(r == 0){
												flag_1 = true;
												// the succ .at(0) is outside the loop, so END will be connected to the true side not the false side of new_branch
												is_false_succ = 0;
										...
										// this succ belongs to no BB so it is either Sink_ or End_
										assert(enode->JustCntrlSuccs->at(r)->type == Sink_ || enode->JustCntrlSuccs->at(r)->type == End_); // sanity check
										switch(enode->JustCntrlSuccs->at(r)->type) {
											case Sink_:
												if(r == 0){
													assert(enode->JustCntrlSuccs->at(1)->type != Sink_);
													if(enode->JustCntrlSuccs->at(1)->BB != nullptr) {
														if(BBMap->at(enode->JustCntrlSuccs->at(1)->BB)->loop != nullptr) {
															assert(BBMap->at(enode->JustCntrlSuccs->at(1)->BB)->loop == BBMap->at(store_node->BB)->loop);
															is_false_succ = 0;
														} else {
															is_false_succ = 1;
														}
													...
													assert(r==1);
													assert(enode->JustCntrlSuccs->at(0)->type != Sink_);
													if(enode->JustCntrlSuccs->at(0)->BB != nullptr) {
														if(BBMap->at(enode->JustCntrlSuccs->at(0)->BB)->loop != nullptr) {
															assert(BBMap->at(enode->JustCntrlSuccs->at(0)->BB)->loop == BBMap->at(store_node->BB)->loop);
															is_false_succ = 1;
														} else {
															is_false_succ = 0;
														}
								...
								if(flag_1 || flag_2) {
									...
									assert(existing_branch->type == Branch_c);
									// the condition is at CntrlPreds->at(1)
									if(existing_branch->CntrlPreds->at(0)->type == Fork_ || existing_branch->CntrlPreds->at(0)->type == Fork_c) {
										new_branch->CntrlPreds->push_back(existing_branch->CntrlPreds->at(0));
										existing_branch->CntrlPreds->at(0)->CntrlSuccs->push_back(new_branch);
									} else {
										assert(existing_branch->CntrlPreds->at(0)->CntrlSuccs->size() == 1); // should have exactly one succ
										assert(existing_branch->CntrlPreds->at(0)->CntrlSuccs->at(0) == existing_branch); // the single succ is simply the existing_branch
										// create a new Fork
										ENode* new_fork = new ENode(Fork_, "fork", store_node->BB);
										new_fork->id = fork_id++;
										enode_dag->push_back(new_fork);
										new_fork->CntrlPreds->push_back(existing_branch->CntrlPreds->at(0));
										existing_branch->CntrlPreds->at(0)->CntrlSuccs->at(0) = new_fork;

										new_fork->CntrlSuccs->push_back(new_branch);
										new_branch->CntrlPreds->push_back(new_fork);

										new_fork->CntrlSuccs->push_back(existing_branch);
										existing_branch->CntrlPreds->at(0) = new_fork;
									}
								...
								for(int r = 0; r < enode->CntrlOrderSuccs->size(); r++) {
									if(enode->CntrlOrderSuccs->at(r)->BB != nullptr) {
										// check if it is outside of any loop
										if(BBMap->at(enode->CntrlOrderSuccs->at(r)->BB)->loop == nullptr) {
											if(r == 0){
												flag_1 = true;
												// the succ .at(0) is outside the loop, so END will be connected to the true side not the false side of new_branch
												is_false_succ = 0;
										...
										// this succ belongs to no BB so it is either Sink_ or End_
										assert(enode->CntrlOrderSuccs->at(r)->type == Sink_ || enode->CntrlOrderSuccs->at(r)->type == End_); // sanity check
										switch(enode->CntrlOrderSuccs->at(r)->type) {
											case Sink_:
											if(r == 0){
												assert(enode->CntrlOrderSuccs->at(1)->type != Sink_);
												if(enode->CntrlOrderSuccs->at(1)->BB != nullptr) {
													if(BBMap->at(enode->CntrlOrderSuccs->at(1)->BB)->loop != nullptr) {
														assert(BBMap->at(enode->CntrlOrderSuccs->at(1)->BB)->loop == BBMap->at(store_node->BB)->loop);
														is_false_succ = 0;
													} else {
														is_false_succ = 1;
													}
												...
												assert(r==1);
												assert(enode->CntrlOrderSuccs->at(0)->type != Sink_);
												if(enode->CntrlOrderSuccs->at(0)->BB != nullptr) {
													if(BBMap->at(enode->CntrlOrderSuccs->at(0)->BB)->loop != nullptr) {
														assert(BBMap->at(enode->CntrlOrderSuccs->at(0)->BB)->loop == BBMap->at(store_node->BB)->loop);
														is_false_succ = 1;
													} else {
														is_false_succ = 0;
													}
			...
			assert(existing_branch != nullptr);

			// connect the successor of the new_branch to end_node.. I identify if it is a true or false succ from the above loop.. I also manage the condition negation of the new_branch in the above loop
			// create a new sink node
			ENode* new_sink = new ENode(Sink_, "sink");
			new_sink->id = sink_id++;
			enode_dag->push_back(new_sink);

			new_sink->CntrlOrderPreds->push_back(new_branch);

			auto pos_ = std::find(end_node->CntrlOrderPreds->begin(), end_node->CntrlOrderPreds->end(), store_node);
			assert(pos_!=end_node->CntrlOrderPreds->end());
			int idx_ = pos_ - end_node->CntrlOrderPreds->begin();
			end_node->CntrlOrderPreds->at(idx_) = new_branch;

			assert(is_false_succ != -1);
			if(is_false_succ == 0) {
				new_branch->CntrlOrderSuccs->push_back(end_node);

				new_branch->CntrlOrderSuccs->push_back(new_sink);
			} else {
				new_branch->CntrlOrderSuccs->push_back(new_sink);
...
void CircuitGenerator::deliver_ACK_tokens_cond_st() {
	ENode* producer = nullptr;
	ENode* consumer = nullptr;
	// loop over the enode_dag in search for a store instruction
	for(auto& enode : *enode_dag) {
		if(enode->type != Inst_) 
			continue; // skip non instructions
		...
		assert(producer->CntrlOrderSuccs->size() == 1);  // making sure that it has a single successor in the CntrlOrder network and that this successor is END_
		assert(producer->CntrlOrderSuccs->at(0)->type == End_);
		consumer = producer->CntrlOrderSuccs->at(0);

		// END_ node is currently not in any BB, so I will change it to be in the very last BB
		consumer->BB = bbnode_dag->at(bbnode_dag->size() - 1)->BB; 

		// Now, we need to decide if the consumer can execute while the producer does not (i.e., if the ST operation executes conditionally)
		////////////////////////////////////////////////////////////////////////////////
		std::ofstream dbg_file;
    	dbg_file.open("st_end_bbs_dep.txt");
        ...
        eliminateCommonControlAncestors(dbg_file, BBMap->at(consumer->BB), BBMap->at(producer->BB), modif_consBB_deps, modif_prodBB_deps); 
        std::vector<std::vector<pierCondition>> f_cons;
        // AYA: since I'm sure that the consumer is END and that it is guaranteed to execute unconditionally, I do not need to construct an SOP for it
        //constructSOP(dbg_file, bbnode_dag->at(0), BBMap->at(consumer->BB), modif_consBB_deps, f_cons); 
        std::vector<std::vector<pierCondition>> f_prod;
        constructSOP(dbg_file, bbnode_dag->at(0), BBMap->at(producer->BB), modif_prodBB_deps, f_prod);
        ...
        if(generate_flag) {
            ...
            ENode* gen_condition_enode = applyShannon(dbg_file, dbg_file, producer->BB, f_gen, is_negated_cond);
            ...
            dbg_file << "\tWill insert a MUX in BB" << BBMap->at(producer->BB)->Idx + 1 << " and the consumer Bx is " << BBMap->at(consumer->BB)->Idx + 1 << "\n\n";
            ENode* gsa_mux = new ENode(Phi_c, "phiC", producer->BB);  // we insert the GENERATE in the producer BB 
            gsa_mux->id = phi_id++;

            gsa_mux->is_st_ack_to_end = true;

            // I commented this because I don't want it to be skipped in the AddSuppress!!
            // gsa_mux->is_mem_dep_component = true;

            gsa_mux->isMux = true;
            gsa_mux->is_store_to_end_gsa = true;
            ...
            gsa_mux->producer_to_branch = producer;
            ...
            for(int uu = 0; uu < producer->CntrlOrderSuccs->size(); uu++) {
                if(producer->CntrlOrderSuccs->at(uu) == consumer) {
                    correct_producer_succ = consumer;
                    break;
                }
            }
            ...
            gsa_mux->CntrlOrderPreds->push_back(producer);
            ...
            int idx_prod_in_cons_pred = returnPosRemoveNode(correct_producer_succ->CntrlOrderPreds, producer); // this function only finds the index of the passed element in the passed vector!
            int idx_cons_in_prod_succ = returnPosRemoveNode(producer->CntrlOrderSuccs, correct_producer_succ);
            correct_producer_succ->CntrlOrderPreds->at(idx_prod_in_cons_pred) = gsa_mux;
            producer->CntrlOrderSuccs->at(idx_cons_in_prod_succ) = gsa_mux;

            // the second input of the gsa_mux should be START!!
            //for(auto& cnt_node : *bbnode_dag->front()->Cntrl_nodes) {
            for(int kk = 0; kk < enode_dag->size(); kk++) {
                if(enode_dag->at(kk)->type == Start_) {
                    gsa_mux->CntrlOrderPreds->push_back(enode_dag->at(kk));
                    enode_dag->at(kk)->CntrlOrderSuccs->push_back(gsa_mux);
            ...
            // AYA: TODO: I'm missing here the logic of delivering START!!

            // AYA: after finishing the above, I need to correctly exit the loop (i.e., add a Branch if the ST is inside a loop!!)
...
void CircuitGenerator::WRONG_deliver_ACK_tokens() {
	ENode* producer = nullptr;
	ENode* consumer = nullptr;
	// loop over the enode_dag in search for a store instruction
	for(auto& enode : *enode_dag) {
        ...
        dbg_file << "\n\n###########################################################\n\n";
        dbg_file << "\n\n\tLet's check modif_prodBB_deps!!\n\n";

        for(int kk = 0; kk < modif_prodBB_deps.size(); kk++) {
        	dbg_file << modif_prodBB_deps.at(kk) + 1 << ", ";
        }
        dbg_file << "\n\n";

        dbg_file << "\n\n\tLet's check modif_consBB_deps!!\n\n";

        for(int kk = 0; kk < modif_consBB_deps.size(); kk++) {
        	dbg_file << modif_consBB_deps.at(kk) + 1 << ", ";
        }
        dbg_file << "\n\n";
		...
		enumerateBBpaths_dfs(BBMap->at(producer->BB), bbnode_dag->at(0), visited, path, path_index, final_paths, false);
		...
		for(int kk = 0; kk < final_paths->size(); kk++) {
			...
			for(int i = 0; i < modif_prodBB_deps.size(); i++) {
				auto pos = std::find(final_paths->at(kk).begin(), final_paths->at(kk).end(), modif_prodBB_deps.at(i));
				if(pos != final_paths->at(kk).end()) {
					...
					pierCondition cond;
					cond.pierBB_index = modif_prodBB_deps.at(i);
					...
					dbg_file << "\n\tThe index of teh pierBB in the final_paths is " << index << " and its value is " << final_paths->at(kk).at(index) + 1 << "\n\n";
					...
					if(index == 0) {
						// extra sanity checks
						assert(bbnode_dag->at(0) == BBMap->at(producer->BB) && BBMap->at(producer->BB)->is_loop_exiting || BBMap->at(producer->BB)->is_loop_header); 
					...
					if(bbnode_dag->at(0) == BBMap->at(producer->BB)) {
						if(BBMap->at(producer->BB)->loop->contains(bbnode_dag->at(modif_prodBB_deps.at(i))->BB) && bbnode_dag->at(modif_prodBB_deps.at(i))->is_loop_exiting) {
							pierCondition cond;
							cond.pierBB_index = modif_prodBB_deps.at(i);
							...
							BBNode* dep_bbnode = bbnode_dag->at(modif_prodBB_deps.at(i));
							////////////////////////////////////////////////////////////
							if(dep_bbnode->loop->contains(dep_bbnode->CntrlSuccs->at(0)->BB)) {
								// sanity check: the other successor must be outside the loop!
								assert(!dep_bbnode->loop->contains(dep_bbnode->CntrlSuccs->at(1)->BB));
...
void CircuitGenerator::buildRedunCntrlNet() {
	for (auto& bbnode : *bbnode_dag) {
		...
		// Search for the start node in this BB 
	        ...
	        phiC_node->id        = phi_id++;
	        phiC_node->isCntrlMg = false;    // AYA: NOte that even if I build this redundant ordered network for the naive LSQ interface, I never need to have cMerges because I do not need them to provide the SEL of MUXes any longer
		...
		// Will call another function in the end of this function to take care of connecting all added branchC to all added Phi_c of the next BB!!
			...
			// AYA: 08/10/2022: commented the following function since I connect the return to void in the beginning using START and I run FPL's algorithm
				// and hardcoded is_void flag to false so that it always connects the phic of the redundnat network to sink!!
...
bool CircuitGenerator::is_negated_cond(int pierBB_index, int next_pierBB_index, BasicBlock* phi_BB, std::ofstream& dbg_file) {
	...
	if(next_pierBB_index == pierBB_index) {
		...
		if(my_post_dom_tree->properlyDominates(bbnode_dag->at(next_pierBB_index)->BB, falseBB) || bbnode_dag->at(next_pierBB_index)->BB == falseBB) {
			// NEW TODOOO: 14/09/2022: Think of how to get the is_negated_cond right in cases where it is post-dominating both the trueBB and the falseBB!!
				// FOR NOW I'm leaving the asesertion to make some examples crash instead of deadlock!!
			// AYA: 14/09/2022: commented the assertion because it could be the case that it is post-dominating both the true and false!!!
...
ENode* CircuitGenerator::addNewMuxes_wrapper(Shannon_Expansion::MUXrep sel_always_mux, BasicBlock* target_bb) {
	...
	new_mux_in0->is_shannons_mux = true;
	new_mux_in1->is_shannons_mux = false;
...
void CircuitGenerator::setMuxes_nonLoop() {
	...
	for(auto & enode : *enode_dag) {
		...
		if((enode->type == Phi_ || enode->type == Phi_n) && (!enode->isMux ) && !enode->is_merge_init ) {  // the is_merge_init serves as the INIT component!!
		...
		if (enode->type == Cst_ && (enode->CntrlPreds->size() + enode->CntrlOrderPreds->size() + enode->JustCntrlPreds->size() == 0)) {
			...
			*/
			...
			if(flag) {  // this means that this enode is a constant that requires to be triggered by a source_node
				...
				enode->is_const_source_triggered = true;
	...
	bool flag;
	 for (auto& enode : *enode_dag) {
        ...
        if (enode->type == Cst_ && (enode->CntrlPreds->size() + enode->CntrlOrderPreds->size() + enode->JustCntrlPreds->size() == 0)) {
			...
			if(!enode->is_const_br_condition && !enode->is_const_feed_memory && !enode->is_redunCntrlNet) {
				if(enode->CntrlSuccs->size() == 1) {
					if(enode->CntrlSuccs->at(0)->type == Inst_)// || (enode->CntrlSuccs->at(0)->type == Phi_ && LI.getLoopFor(enode->CntrlSuccs->at(0)->BB) == nullptr))  // TEMPORARY ADDING THE EXTRA OR!!!
						flag = true;
				} else {
	...
	*/
	...
	for(auto& enode : *enode_dag) {
		if( (enode->type == Cst_ /*&& enode->BB == startC_node->BB*/)) {
			...
			if(enode->JustCntrlPreds->size() == 0 && enode->CntrlPreds->size() == 0) {
...
// Aya: 08/10/2022: this function should be called in the very beginnging before applying the FPL'22 algorithm
void CircuitGenerator::connectReturnToVoid_irredundant() {
	ENode* return_to_void = nullptr;
	bool is_void = false;
	for(auto& enode : *enode_dag) {
		if(enode->type == Inst_ && enode->Name.compare("ret") == 0 && enode->CntrlPreds->size() == 0) {
			return_to_void = enode;
			is_void = true;
			break;
		}
	}
	...
	if(is_void) {
		assert(return_to_void != nullptr);

		// search for the START node to feed the return
		for(auto& enode : *enode_dag) {
			if(enode->type == Start_) {
				return_to_void->JustCntrlPreds->push_back(enode);
				enode->JustCntrlSuccs->push_back(return_to_void);
				break;
			}
		}
	} 
