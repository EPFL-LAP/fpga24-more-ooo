...
void CircuitGenerator::FindLoopDetails(LoopInfo& LI) {
	...
	// AYA: 20/09/2023: added the following to mark BBs that belong to innermost loops
	int smallest_id = 100;
	int corresponding_i = -1;
	for(int i = 0; i < bbnode_dag->size(); i++) {
		if(bbnode_dag->at(i)->loop == nullptr)
			continue;

		// the innermost loop is the loop that has the loop exit with the smallest BB Idx
		if(bbnode_dag->at(i)->is_loop_exiting || bbnode_dag->at(i)->is_loop_latch) {
			if(bbnode_dag->at(i)->Idx < smallest_id) {
				smallest_id = bbnode_dag->at(i)->Idx;
				corresponding_i = i;
			}
		}

		//std::vector<Loop*> sub_loops = bbnode_dag->at(i)->loop->getSubLoopsVector();
		//if(sub_loops.size() == 0)
	}

	assert(smallest_id < 100 && corresponding_i != -1);
	Loop* inner_most_loop = bbnode_dag->at(corresponding_i)->loop;
