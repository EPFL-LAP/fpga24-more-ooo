...
int CircuitGenerator::bx_buffer_wrapper_id  = 0;
int CircuitGenerator::bx_join_wrapper_id  = 0;

int CircuitGenerator::lazy_fork_id = 0;
...
int CircuitGenerator::rob_id = 0;  // AYA: 07/08/2023

int CircuitGenerator::tagger_id = 0;  // AYA: 17/09/2023

int CircuitGenerator::un_tagger_id = 0;  // AYA: 18/09/2023

int CircuitGenerator::free_tags_fifo_id = 0;

int CircuitGenerator::aligner_branch_id = 0;
int CircuitGenerator::aligner_mux_id = 0;
...
void CircuitGenerator::addBuffersSimple_OLD() {
     ...
     for (auto& enode : *enode_dag) {
         if (enode->type == Phi_ || enode->type == Phi_n || enode->type == Phi_c || enode->type == Loop_Phi_n || enode->type == Loop_Phi_c) {
         	if(enode->is_non_loop_gsa_mux)  // skip if the phi_c is inserted in the smart LSQ to mimic a non loop gsa !
         		continue;
		// Aya added an extra Or in the following condition!!
             if (enode->CntrlPreds->size() > 1 || enode->JustCntrlPreds->size() > 1 || enode->CntrlOrderPreds->size() > 1) {
				...
				*/ 
                 ...
                 if (enode->type == Phi_c || enode->type == Loop_Phi_c) {
					...
					if(enode->is_redunCntrlNet) {
						...
						// AYA: 14/09/2022: added the CntrlOrder network to the condition!!!
						assert(enode->JustCntrlSuccs->size() > 0 || enode->CntrlOrderSuccs->size() > 0);
					...
					assert(enode->type == Phi_ || enode->type == Phi_n || enode->type == Loop_Phi_n);
...
void CircuitGenerator::constructDomTree(Function& F) {
	 my_dom_tree = new DominatorTree(F);
}
...
void CircuitGenerator::Aya_addFork() {
	std::vector<ENode*>* new_forks = new std::vector<ENode*>;
	for(auto& enode: *enode_dag) {
		if(enode->CntrlSuccs->size() + enode->JustCntrlSuccs->size() > 1 
			...
			&& enode->type != MC_ && enode->type != LSQ_ && !enode->isLoadOrStore() && enode->BB != nullptr
				...
				&& !enode->is_data_loop_cmerge
				&& enode->type != Tagger
				&& enode->type != Un_Tagger
				&& enode->type != Free_Tags_Fifo) {  // AYA: 16/09/2023: ADDED the last and to skip the CNtrlMerges that I newly added in loops because I will handle them separately at the end of the function
		...
		// AYA: 17/09/2023: handling the new CMerges
			// AYA: 18/09/2023: extended it to support CMerges of if-then-else
		if(enode->is_data_loop_cmerge || enode->is_if_else_cmerge) {
			assert(enode->isCntrlMg);
			assert(enode->type == Phi_ || enode->type == Phi_n);
			addCMergeSuccs(enode);
		}
		...
		if(enode->type == Branch_n) {
			...
			if(enode->type == Branch_c && !enode->is_redunCntrlNet) {
				...
				if(enode->true_branch_succs_Ctrl->size() > 0 || enode->false_branch_succs_Ctrl->size() > 0) {
					...
					// Aya: 12/09/2022: since I want to support two modes of operation on the CntrlOrder, the following should be done only if the 
							// branch component is not labelled as a redundant component!
					if(!enode->is_redunCntrlNet) {
...
// AYA: 17/09/2023
void CircuitGenerator::addCMergeSuccs(ENode* cmerge_node) {
	// The convention is that a CMerge should have two outputs: out1 carries data from the inputs, out2 carries the condition corresponding to which input is accepted
	
	// 1st: let's do some sanity checks
	assert(cmerge_node->cmerge_data_succs->size() > 0);
	//assert(cmerge_node->cmerge_control_succs->size() > 0);  // commented it out because it is not necessarily the case that a CMerge has a control succ
	assert(cmerge_node->CntrlSuccs->size() == 0);

	// let's figure out what sould be the first succ out1
	if(cmerge_node->cmerge_data_succs->size() == 1) {
		cmerge_node->CntrlSuccs->push_back(cmerge_node->cmerge_data_succs->at(0));
	} else {
		// create a new Fork node 
		ENode* fork = new ENode(Fork_, "fork", cmerge_node->BB);
		fork->id = fork_id++;
		enode_dag->push_back(fork);

		fork->CntrlPreds->push_back(cmerge_node);
		cmerge_node->CntrlSuccs->push_back(fork);

		// let the fork feed all of the data succs of the cmerge
		for(int i = 0; i < cmerge_node->cmerge_data_succs->size(); i++) {
			fork->CntrlSuccs->push_back(cmerge_node->cmerge_data_succs->at(i));

			// insert the fork at the pred of this succ in place of the cmerge
			auto pos = std::find(cmerge_node->cmerge_data_succs->at(i)->CntrlPreds->begin(), cmerge_node->cmerge_data_succs->at(i)->CntrlPreds->end(), cmerge_node);
			assert(pos != cmerge_node->cmerge_data_succs->at(i)->CntrlPreds->end());
			int idx = pos - cmerge_node->cmerge_data_succs->at(i)->CntrlPreds->begin();
			cmerge_node->cmerge_data_succs->at(i)->CntrlPreds->at(idx) = fork;
		}
	}

	// now, let's figure out what sould be the second succ out2
	if(cmerge_node->cmerge_control_succs->size() == 1) {
		cmerge_node->CntrlSuccs->push_back(cmerge_node->cmerge_control_succs->at(0));
	} else {
		if(cmerge_node->cmerge_control_succs->size() > 0) {
			// create a new Fork node 
			ENode* fork = new ENode(Fork_, "fork", cmerge_node->BB);
			fork->id = fork_id++;
			enode_dag->push_back(fork);
			...
			// let the fork feed all of the data succs of the cmerge
			for(int i = 0; i < cmerge_node->cmerge_control_succs->size(); i++) {
				fork->CntrlSuccs->push_back(cmerge_node->cmerge_control_succs->at(i));

				// insert the fork at the pred of this succ in place of the cmerge
				auto pos = std::find(cmerge_node->cmerge_control_succs->at(i)->CntrlPreds->begin(), cmerge_node->cmerge_control_succs->at(i)->CntrlPreds->end(), cmerge_node);
				assert(pos != cmerge_node->cmerge_control_succs->at(i)->CntrlPreds->end());
				int idx = pos - cmerge_node->cmerge_control_succs->at(i)->CntrlPreds->begin();
				cmerge_node->cmerge_control_succs->at(i)->CntrlPreds->at(idx) = fork;
			}
		} else {
			// this second output of the CntrlMerge that is the condition output should feed a sink then
			ENode* sink_node = new ENode(Sink_, "sink");
			sink_node->id    = sink_id++;
			enode_dag->push_back(sink_node);

            cmerge_node->CntrlSuccs->push_back(sink_node);
            sink_node->CntrlPreds->push_back(cmerge_node);
...
void CircuitGenerator::extraCheck_AddBranchSuccs() {
	for(auto& enode : *enode_dag) {
		if(enode->type != Branch_c) 
			continue;

		// at this point, I'm sure that the enode that arrives here is of type Branch_c
		if(enode->true_branch_succs_Ctrl->size() + enode->false_branch_succs_Ctrl->size() > 0) {
			assert(enode->true_branch_succs_Ctrl_lsq->size() + enode->false_branch_succs_Ctrl_lsq->size() == 0);
			// making sure that no one took care of it earlier
			assert(enode->CntrlSuccs->size() + enode->JustCntrlSuccs->size() + enode->CntrlOrderSuccs->size() == 0);
			// the convention is succ.at(0) is the true and succ.at(1) is the false
			switch(enode->true_branch_succs_Ctrl->size()) {
				case 0: {
					// connect a sink to the true succ
					ENode* sink_node = new ENode(Sink_, "sink");
					sink_node->id = sink_id++;
					enode_dag->push_back(sink_node);
					...
					// there must be one in the false side
					assert(enode->false_branch_succs_Ctrl->size() > 0);
					switch(enode->false_branch_succs_Ctrl->size()) {
						case 1: {
							// connect it directly.. the Branch enode is already in the preds of the node...
							enode->JustCntrlSuccs->push_back(enode->false_branch_succs_Ctrl->at(0));
						}
						break;

						default: // which is any size > 1
							// insert a fork
							ENode* fork_node = new ENode(Fork_c, "forkC", enode->BB);
							fork_node->id = fork_id++;
							enode_dag->push_back(fork_node);

							fork_node->JustCntrlPreds->push_back(enode);
							enode->JustCntrlSuccs->push_back(fork_node);
							for(int j = 0; j < enode->false_branch_succs_Ctrl->size(); j++) {
								fork_node->JustCntrlSuccs->push_back(enode->false_branch_succs_Ctrl->at(j));
								auto pos = std::find(enode->false_branch_succs_Ctrl->at(j)->JustCntrlPreds->begin(), enode->false_branch_succs_Ctrl->at(j)->JustCntrlPreds->end(), enode);
								assert(pos != enode->false_branch_succs_Ctrl->at(j)->JustCntrlPreds->end());
				...
				case 1: {
					// connect it directly
					enode->JustCntrlSuccs->push_back(enode->true_branch_succs_Ctrl->at(0));
				...
				default: {
					// which is any size > 1
					// for extra checking
					assert(enode->true_branch_succs_Ctrl->size() > 1);
					...
					fork_node->JustCntrlPreds->push_back(enode);
					enode->JustCntrlSuccs->push_back(fork_node);
					for(int j = 0; j < enode->true_branch_succs_Ctrl->size(); j++) {
						fork_node->JustCntrlSuccs->push_back(enode->true_branch_succs_Ctrl->at(j));
						auto pos = std::find(enode->true_branch_succs_Ctrl->at(j)->JustCntrlPreds->begin(), enode->true_branch_succs_Ctrl->at(j)->JustCntrlPreds->end(), enode);
						assert(pos != enode->true_branch_succs_Ctrl->at(j)->JustCntrlPreds->end());
			...
			assert(enode->true_branch_succs_Ctrl_lsq->size() + enode->false_branch_succs_Ctrl_lsq->size() > 0);
			assert(enode->CntrlOrderSuccs->size() != 0); // I'm assuming that this one is taken care of already above!!
		}
...
void CircuitGenerator::addBrSuccs(ENode* branch, networkType network_flag) {
	...
	if(network_flag != constCntrl)
		sink_node = new ENode(Sink_, "sink"); //, enode->BB);
	...
	if(network_flag == constCntrl)  // AYA: 03/08/2023: disabled this for the JUstCNtrl network due to a weird LLVM error.. Moved it to a later function called in MyCFGPass
		return;
