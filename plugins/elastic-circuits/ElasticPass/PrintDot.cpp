...
void aya_printDotDFG(std::vector<ENode*>* enode_dag, std::vector<BBNode*>* bbnode_dag, std::string name, std::string serial_number, bool fix_mc_st_interfaces_flag) {
    ...
    */
    aya_printDotNodes(enode_dag, serial_number, bbnode_dag, fix_mc_st_interfaces_flag);
	
	aya_printotherNodes(enode_dag, serial_number, fix_mc_st_interfaces_flag);  // print the nodes that are not inside any BB!
	dotfile <<"\n";

    aya_printDotEdges(enode_dag, bbnode_dag, fix_mc_st_interfaces_flag);
...
void aya_printDotEdges(std::vector<ENode*>* enode_dag, std::vector<BBNode*>* bbnode_dag, bool fix_mc_st_interfaces_flag) {
    dotfile << printMemEdges(enode_dag, fix_mc_st_interfaces_flag);
    dotfile << aya_printDataflowEdges(enode_dag, bbnode_dag, fix_mc_st_interfaces_flag);
}
...
std::string aya_printDataflowEdges(std::vector<ENode*>* enode_dag, std::vector<BBNode*>* bbnode_dag, bool fix_mc_st_interfaces_flag) {
	...
	*/ 
	...
	// AYA: 01/10/2023:
	std::vector<std::vector<ENode*>> already_printed_idxs;  // this vector keeps track of the already printed indices when the enode and the enode_succ happen to have multple connections to each other
												// for instance, the case of dirty nodes fed by the UN_TAGGER and the connections between the ALigner_Branch and Aligner_Mux
	...
	for (auto& enode : *enode_dag) {
		...
		already_printed_idxs.clear();  // AYA: 01/10/2023
		...
		// loop over all the data path successors of one enode	
 		for (int y = 0; y < enode->CntrlSuccs->size(); y++) {
 			ENode* enode_succ = enode->CntrlSuccs->at(y);

 			/////////////////////// AYA: 01/10/2023: identify where to push this enode in already_printed_idxs
 			bool found = false;
 			for(int kk = 0; kk < already_printed_idxs.size(); kk++) {
 				assert(already_printed_idxs.at(kk).size() > 0);
 				if(already_printed_idxs.at(kk).at(0) == enode_succ) {
 					already_printed_idxs.at(kk).push_back(enode_succ);
 					found = true;
 					break;
 				}
 			}
 			if(!found) {   // create a new entry for it
 				std::vector<ENode*> temp;
 				temp.push_back(enode_succ);
 				already_printed_idxs.push_back(temp);
 			}
 			////////////////////////////////////////////////////////////////////////////////
			...
			if(enode_succ->type != LSQ_ && enode_succ->type != MC_ && enode->type != LSQ_ && enode->type != MC_ && enode->type != Fork_) {
				...
				if((enode->type == Phi_c && enode->is_redunCntrlNet)
					...
					//if(enode->type == Fork_) {
					//} else {
						// AYA: 30/09/2023: added a condition to check if the enode->CntrlSuccs is "dirty_node", this means that its two
									// inputs are passed from the un_tagger, but the indexOf function will always output the first occurence only!!
							// the same story could occur where the TAGGER feeds the two inputs of one node	
						if(enode_succ->is_dirty_node || (enode->type == Tagger && (enode_succ->type == Branch_n || enode_succ->type == Branch_c))
								|| enode_succ->type == Aligner_Mux) {
							if(enode_succ->is_dirty_node)
								assert(enode->type == Un_Tagger);
							// do the seach for the enode_succ in the CntrlSuccs manually..
							str += ", from = \"out" + to_string(y + 1) + "\"";
						} else {
...
//////////////////////////////////////////////////////////////////////////////////////////	
				...
				if(enode_succ->type == Inst_) {
					if(isa<LoadInst>(enode_succ->Instr)){
						...
						// AYA: 30/09/2023: added a condition to check if the enode->CntrlSuccs is "dirty_node", this means that its two
									// inputs are passed from the un_tagger, but the indexOf function will always output the first occurence only!!
						if(enode_succ->is_dirty_node/* || enode->type == Tagger*/) {
							// IMP ASSUMPTION: I assume that a dirty_node has only two inputs and here I check, if y == indexOf this means we are at the first connection between
									// the dirty_node and the un_tagger, else, then we are at the second occurance and thus must not take indexOf here..
							if(enode_succ->is_dirty_node)
								assert(enode->type == Un_Tagger);
							int correct_idx;
							for(int z = 0; z < enode_succ->CntrlPreds->size(); z++) {
								if(enode_succ->CntrlPreds->at(z) == enode) {
									correct_idx = z;
									if(y == indexOf(enode->CntrlSuccs, enode_succ)) {
										break;   // stop at the first occurance; otherwise, continue normally and stop at the last occurance
									}
								}
							}
							str += ", to = \"in" + to_string(correct_idx + 1) + "\"";
							str += "];\n";
						} else {
					...
					if(enode_succ->type == End_) {
						...
						if(enode_succ->type == Branch_c) {
							...
							if(enode_succ->is_dirty_node || (enode->type == Tagger && (enode_succ->type == Branch_n || enode_succ->type == Branch_c))
								...
								// AYA: 01/10/2023: commented the previous code and replaced it with the following that does not assume the number of preds or succs
								if(enode_succ->is_dirty_node)
									assert(enode->type == Un_Tagger);
								int occurance_num = -1;
								int correct_idx;
								for(int kk = 0; kk < already_printed_idxs.size(); kk++) {
									assert(already_printed_idxs.at(kk).size() > 0);
					 				if(already_printed_idxs.at(kk).at(0) == enode_succ) {
				 						occurance_num = already_printed_idxs.at(kk).size();
					 				}
								}
								assert(occurance_num != -1);

								int temp_count = 0;
								for(int z = 0; z < enode_succ->CntrlPreds->size(); z++) {
									if(enode_succ->CntrlPreds->at(z) == enode) {
										temp_count++;
										if(temp_count == occurance_num) {
											correct_idx = z;
											break;   
										}
									}
...
// The assumption is that predecessors and successors are placed at the correct indices
			...
			if(enode->type == Fork_ && fork_data_index != -1) {
				...
				if(enode->type == LoopMux_Synch || enode->type == Tagger || enode->type == Un_Tagger)  // AYA: 30/04/2023: for the LoopMUX_Synch  // AYA: 17/09/02023: this applies to the TAGGER as well
...
//////////////////////////////////////////////////////////////////////
			...
			// Aya: 08/10/2021: Added the following condition to correctly increment the inputs of the inputs of getelementptr node which has some inputs in the data network and others in the JustCntrl network
				...
				if(enode_cntrl_succ->type == Inst_) {
					...
					// AYA: 17/09/2023: did the same for the TAGGER as well
					if((enode_cntrl_succ->type == Phi_c && !enode_cntrl_succ->is_redunCntrlNet) || enode_cntrl_succ->type == Loop_Phi_c
							 || enode_cntrl_succ->type == LoopMux_Synch || enode_cntrl_succ->type == Tagger || enode_cntrl_succ->type == Un_Tagger) {
						str += ", to = \"in" + to_string(indexOf(enode_cntrl_succ->JustCntrlPreds, enode) + enode_cntrl_succ->CntrlPreds->size() + 1) + "\"";
						str += "];\n";
					} else {
...
///////////////////////////////////////////////////// NEW REDUNDANT CONTROL NETWORK STUFF!!!
		...
		// Aya: 15/09/2022: the naming enode_redun_cntrl_succ is confusing because we add irredundant components in the CntrlOrder network if we do the smart LSQ interfacing!
		// loop over all the new control path successors of one enode
 		for (auto& enode_redun_cntrl_succ : *(enode->CntrlOrderSuccs)) {
			if ( (enode_redun_cntrl_succ->type != LSQ_ && enode_redun_cntrl_succ->type != MC_ && enode->type != LSQ_ && enode->type != MC_) ||
					 ((enode->type == Fork_c || enode->type == LazyFork_) && enode_redun_cntrl_succ->type == LSQ_) || (enode->type == Bx_Join_wrapper_ && enode_redun_cntrl_succ->type == LSQ_) || 
					 (enode->type == Branch_c && enode_redun_cntrl_succ->type == LSQ_) || 
					 (enode->type == Phi_c && enode_redun_cntrl_succ->type == LSQ_ && enode->is_non_loop_gsa_mux)  ) {
				...
				if(enode->type == Fork_ && fork_data_index != -1 && fork_control_index != -1) {
					...
					if(enode->type == Fork_ && fork_data_index != -1) {
						...
						if(enode->type == Fork_c && fork_control_index != -1){
							...
							if(enode->type == LoopMux_Synch || enode->type == Tagger || enode->type == Un_Tagger )  // AYA: 30/04/2023: for the LoopMUX_Synch   // AYA: 17/09/2023: the same applies for the TAGGER
								...
								if(enode->type == Inst_ && fix_mc_st_interfaces_flag) {   // AYA: 11/07/2023: added the following condition to correctly index the output of the new ST
									if(isa<StoreInst>(enode->Instr)) {
										str += ", from = \"out" + to_string(indexOf(enode->CntrlOrderSuccs, enode_redun_cntrl_succ) + 2 + enode->CntrlSuccs->size() + enode->JustCntrlSuccs->size()) + "\"";
									} else {
				...
				// Aya: 15/09/2022: to properly output for the Phi_c MUXes (not CMerges) that are added by the smart LSQ interfacing!!
				if((enode_redun_cntrl_succ->type == Phi_c && !enode_redun_cntrl_succ->is_redunCntrlNet) || enode_redun_cntrl_succ->type == Loop_Phi_c) {
					// assert that it must be a MUX!!
					//assert(enode_redun_cntrl_succ->isMux);
					str += ", to = \"in" + to_string(indexOf(enode_redun_cntrl_succ->CntrlOrderPreds, enode) + enode_redun_cntrl_succ->CntrlPreds->size() + 1) + "\"";
					str += "];\n";
				} else {
						// which input it is comes from the position of the the node in the successor's CntrlOrderPreds array
...
void aya_printotherNodes(std::vector<ENode*>* enode_dag, std::string serial_number, bool fix_mc_st_interfaces_flag) {
		...
		for (auto& enode : *enode_dag) {
			...
			if(!skipNodePrint(enode) && enode->BB == nullptr){
				...
				if (OptimizeBitwidth::isEnabled()) {
					...
					dotline += getNodeDotInputs(enode, fix_mc_st_interfaces_flag);
					dotline += getNodeDotOutputs(enode, fix_mc_st_interfaces_flag);
				}
				/////////////////////
...
void aya_printDotNodes(std::vector<ENode*>* enode_dag, std::string serial_number, std::vector<BBNode*>* bbnode_dag, bool fix_mc_st_interfaces_flag) {
// Target is to loop over basic blocks and print the operations existing in each basic block
	string dotline = "";
	int count = 0;
	...
	for (auto& bnd : *bbnode_dag) {
		...
		for (auto& enode : *enode_dag) {
			...
			if((enode->BB == bnd->BB) && !skipNodePrint(enode)){
				...
				dotline += "\", ";
				dotline += aya_getNodeDotbbID(bnd);
				// Aya: 14/09/2022: included the Bx_Buffer_wrapper_ to my condition!!
				if (enode->type == Inst_ || enode->type == Bx_Buffer_wrapper_ || enode->type == Bx_Join_wrapper_)
					dotline += getNodeDotOp(enode);
////////////////////
				if (OptimizeBitwidth::isEnabled()) {
					dotline += getNodeDotInputs_withOB(enode);
...
/////////////////////
				...
				// AYA: 05/08/2023: added the following to forward information about components that need to be tagged
				dotline += ", tagged=";
				if(enode->is_tagged)
					dotline += "true";
				else
...
int getInPortSize(ENode* enode, int index) {
    ...
    if (enode->type == Argument_ || enode->type == Aligner_Mux || enode->type == Aligner_Branch) {  // AYA: 01/10/2023
        return DATA_SIZE;
    ...
    } else if (enode->type == Fork_c || enode->type == LazyFork_) {  // Aya: 04/11/2022: added Lazy fork in the condition because they are used in the LSQs
        return CONTROL_SIZE;
    } else if (enode->type == Buffera_ || enode->type == Bufferi_ || enode->type == Fifoa_ ||
               enode->type == Fifoi_) {
        return enode->CntrlPreds->size() > 0 ? DATA_SIZE : CONTROL_SIZE;

    } else if (enode->type == Phi_ || enode->type == Phi_n || enode->type == Loop_Phi_n) {  // AYA: 28/02/2023: added the last OR
...
// AYA: Added a condition to handle the case of an irredundant Phi_c becoming a mux
    } else if (enode->type == Phi_c || enode->type == Loop_Phi_c) {  // AYA: 28/02/2023: added the last OR
		if(enode->is_redunCntrlNet) {
			return CONTROL_SIZE;
		} else {
...
std::string getNodeDotTypeNew(ENode* enode) {
    ...
    switch (enode->type) {
        ...
        case Phi_n:
            if (enode->isMux) {
            	assert(!enode->is_data_loop_cmerge);
                name += "Mux";
            }
            else {
            	if(enode->is_data_loop_cmerge || enode->is_if_else_cmerge) {
        ...
        case Phi_c:
            ...
            break;
		// Aya: 28/02/2023: adding support for the special mux that we will insert at the loop headers that does not require INIT at its SEL
        case Loop_Phi_n:
        case Loop_Phi_c:
        	name += "LoopMux";
        break;
		...
		case Inj_n:
			name += "Inj";
			break;
		// Aya: 04/11/2022: added support for printing lazyForks that are created in the LSQ pass
		case LazyFork_:
			name += "LazyFork";
			break;
		case ROB:
...
std::string getNodeDotOp(ENode* enode) {
    string name = ", op = \"";
    // Aya: 14/09/2022: added the following if, else condition to account for me new Bx_Buffer_wrapper_ component that is treated as an operation!!
    if(enode->type == Bx_Buffer_wrapper_ || enode->type == Bx_Join_wrapper_) {
    	if(enode->type == Bx_Buffer_wrapper_)
    		name += "buffer_bx_op\"";
    	else {
    		assert(enode->type == Bx_Join_wrapper_);
    		name += "join_bx_op\"";
    	}
    } else {
...
std::string getNodeDotInputs(ENode* enode, bool fix_mc_st_interfaces_flag) {
    string name = "";
    ...
    switch (enode->type) {
    	case ROB:
    		...
    		for(int i = 0; i < enode->CntrlPreds->size(); i++) {
	            ...
	            name += ":" + to_string(DATA_SIZE);   // the 2nd input is the tag which has less bits than DATA_SIZE but this will be fixed later!!
    		}
    		name += "\"";
		break;
    	case LoopMux_Synch:
    	case Tagger:   // AYA:17/09/2023
    	case Un_Tagger:
    	case Free_Tags_Fifo:
        ...
        case Start_:
        case Sink_:
        case LazyFork_: // Aya: 04/11/2022: to add support for the lazy forks that are inserted for LSQs!
        ...
        case Branch_n:
        case Branch_c:
        case Aligner_Branch:   // AYA: 30/09/2023
            name += ",  in = \"in1:" + to_string(getInPortSize(enode, 0));
        ...
        case Loop_Phi_c:
			...
			if( (enode->type == Phi_c && enode->is_redunCntrlNet) /*|| (enode->type == Inst_ && enode->Name.compare("ret") == 0 && enode->CntrlPreds->size() == 0) */ ) {
	        	...
	        	// Aya: 14/09/2022: Included this CntrlOrderPreds in the story because I can no have them as nonredundant in the smart LSQ network!!

					// Aya added this line!!
	 			if (enode->CntrlPreds->size() > 0 || enode->JustCntrlPreds->size() > 0 || enode->CntrlOrderPreds->size() > 0)
		             name += ", in = \"";
		        // name += ", in = \"";
				...
				if(enode->is_advanced_component) {
			       ...
			       for (int i = 0; i < (int)enode->CntrlPreds->size() + (int)enode->JustCntrlPreds->size() + (int)enode->CntrlOrderPreds->size(); i++) {
				        ...
				        name += ":";
				        if(enode->type == Inst_) {
				        	if(isa<StoreInst>(enode->Instr) && i == 2) {
				        		name += "0 ";
				        	} else {
						        name += to_string(getInPortSize(enode, i)) + " ";
				        	}
        ...
        case MC_:
		  ...
		  */
            ...
            // AYA: 18/07/2023: Made the following code conditional because eventually I do not want to feed the MC with the constant that counts stores
        ...
        case End_:
			...
			for (auto& pred : *enode->CntrlOrderPreds) {
                ...
                name += (pred->type == MC_ || pred->type == LSQ_ || 
                			(pred->type == Inst_ && isa<StoreInst>(pred->Instr) && fix_mc_st_interfaces_flag) ||
                				(pred->type == Phi_c && pred->is_store_to_end_gsa) ||
                				(pred->type == Branch_c && pred->producer_to_branch->type == Inst_ && isa<StoreInst>(pred->producer_to_branch->Instr))  // AYA: 02/08/2023: added another extra OR to support ST that are inside a loop so feed END through a Branch
            				) ? "*e " : "";  // AYA" 12/07/2023: added an extra OR to the condition to support the new ST and MC
            }
...
std::string getNodeDotOutputs(ENode* enode, bool fix_mc_st_interfaces_flag) {
    string name = "";
    ...
    switch (enode->type) {
    	case ROB:
    		assert(enode->CntrlSuccs->size() == 1);
    		name += ", out = \"";
    		name += " out" + to_string(1) + ":" + to_string(DATA_SIZE);
    		name += "\"";
    	break;
    	case LoopMux_Synch:
    	case Tagger: // AYA: 17/09/2023
    	case Un_Tagger:
    	case Free_Tags_Fifo:
    	case Aligner_Branch:   // AYA: 01/10/2023
    	case Aligner_Mux:
			if (enode->CntrlSuccs->size() > 0)
				name += ", out = \"";
        ...
        case Phi_:
        case Phi_n:
        case Loop_Phi_n: // Aya: 28/02/2023
        case Source_:
		case Inj_n:
            name += ", out = \"out1:" + to_string(getOutPortSize(enode, 0));
            if(enode->type == Loop_Phi_n)  { // Aya: 07/08/2023
            	if(enode->CntrlSuccs->size() > 1) {
            		assert(enode->CntrlSuccs->size() == 2);
            		name += " out2:" + to_string(DATA_SIZE); // It should be tag size but this should be fixed later...
            	}
            } else {
            	// AYA: 16/09/2023: added this to consider CMerges of type Phi_n (i.e., carrying data)
            	if(enode->is_data_loop_cmerge || enode->is_if_else_cmerge) {
            		assert(enode->isCntrlMg);
            		assert(enode->CntrlSuccs->size() == 2);
        ...
        case Fork_c:
        case Inst_:
        case LazyFork_: // Aya: 04/11/2022: added the following to support lazy forks that are added for LSQs!!
			...
			if (enode->type == Inst_)
                if (enode->Instr->getOpcode() == Instruction::Store) {
					// Aya added this line!
                    //name += ", out = \"";
					if(fix_mc_st_interfaces_flag && enode->CntrlOrderSuccs->size() > 0) {
						name += "out3:0";      // AYA: 11/07/2023: this line is applicable only in the new designs of ST and MC
					} else {
						name += "out2:32 ";
					}
					// Aya added this line!!
        ...
        case Phi_c:
        case Loop_Phi_c: // Aya: 28/02/2023
        ...
        case MC_:
            ...
            if (enode->type == MC_ && enode->lsqToMC == true)
            	...
            	//if(!(enode->type == MC_ && enode->CntrlOrderSuccs->size() > 0)) {
            	if(enode->type != MC_) {
            ...
            //////////////////////// AYA: 17/07/2023: added the following to support new ACK outputs from the MC (1 ACK per ST)
            // I connect those ACK outputs to the CntrlOrderSuccs of the MC and it should not contain anything except this MC!!
            if(enode->type == MC_) {
            	if(enode->CntrlOrderSuccs->size() > 0) {
	            	/*int number_of_stores = 0;   
	            	for(int i = 0; i < enode->CntrlPreds->size(); i++) {
	            		if( (enode->CntrlPreds->at(i)->type == Instr && isa<StoreInst>(enode->CntrlPreds->at(i)->Instr) ) || enode->CntrlPreds->at(i)->type == Cst_) {
	            			number_of_stores++;
	            		}
	            	}*/
	            	// AYA: 24/07/2023: temporarily placing it as e instead of f because this is what is supported by the vhdl_writer
	            	for(int i = 0; i < enode->CntrlOrderSuccs->size(); i++) {  
		                name += "out" + to_string (1 + i + getMemOutputCount(enode)) + ":" + to_string(0);
            		...
            		// AYA: 24/07/2023: if there are no ST, the CntrlOrderSuccs (by construction) will be of size 0, but we need to still print this extra port and connect it to END
	                name += "out" + to_string (1 + getMemOutputCount(enode)) + ":" + to_string(0) + "*f ";//+ "*s" + to_string(i) + "f "
            	} */
            }
            /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
std::string inputSuffix (ENode* enode, unsigned idx) {    
    switch (enode->type)
    {
 	case Aligner_Mux: // AYA: 30/09/2023
    	if (idx == MUX_CONDITION_IN)
            return "?";
        return "";
    case Inst_:
        if (enode->Instr->getOpcode() == Instruction::Select) {
...
int getInPortSize_withOB(ENode* enode, unsigned index) {
    ...
    case Loop_Phi_c: // Aya: 28/02/2023
        ...
        // AYA: 08/11/2022: I no longer have CMerges so I will never enter this condition!
...
std::string getNodeDotInputs_withOB(ENode* enode) {
    ...
    switch (enode->type) {
        ...
        case Start_:
        case Sink_:
        case LazyFork_: // Aya: 08/11/2022: to add support for the lazy forks that are inserted for LSQs!
        ...
        case Loop_Phi_c:
			...
			if( (enode->type == Phi_c && enode->is_redunCntrlNet) /*|| (enode->type == Inst_ && enode->Name.compare("ret") == 0 && enode->CntrlPreds->size() == 0) */ ) {
				...
				if(enode->is_advanced_component) {
		        	...
		        	// Aya: 14/09/2022: Included this CntrlOrderPreds in the story because I can have them as nonredundant in the smart LSQ network!!

				} 

				// I handle the entirety of the Load above!
		        /*if (enode->type == Inst_)
		            if (enode->Instr->getOpcode() == Instruction::Load)  // need to do something extra for the load instructions!
		                name += "in2:32"; */
        	...
        	// AYA: 08/11/2022: commented the following code and replaced it with the above code that I got from the other getNodeDotInputs
...
std::string getNodeDotOutputs_withOB(ENode* enode) {
    ...
    switch (enode->type) {
        ...
        case Start_:
        case Source_:
        case End_:   // check if I should move it to a separate case
        ...
        case LazyFork_: // Aya: 04/11/2022: added the following to support lazy forks that are added for LSQs!!
			...
			name += "\"";
        	// replaced the following code with the code the above
            /*name += ", out = \"";
...
getFloatValue(get_component_delay(enode->Name, DATA_SIZE, serial_number, VD)) + "\"";
            ...
            name += ", II=1";
            break;
        // Aya: 14/09/2022: added the following for my new Buffer_Bx wrapper component
        case Bx_Buffer_wrapper_:
        case Bx_Join_wrapper_:
        	name += ", delay=0";
            name += ", latency=0"; // latency=1?
            name += ", II=1";
        	break;
        case Phi_:
		if ( enode->isMux )
        ...
        if (!skipNodePrint(enode)) {
            ...
            if (full) {
                ...
                if (OptimizeBitwidth::isEnabled()) {
                    ...
                    dotline += getNodeDotInputs(enode, false);
                    dotline += getNodeDotOutputs(enode, false);
                }
                dotline += getNodeDotParams(enode, serial_number);
                if (enode->type == MC_ || enode->type == LSQ_)
                    dotline += getNodeDotMemParams(enode);
...
std::string printColor(ENode* from, ENode* to) {
    ...
    else if (from->type == Phi_c || from->type == Loop_Phi_c || from->type == Branch_c || from->type == Fork_c || from->type == LazyFork_ ||
             to->type == Phi_c || to->type == Loop_Phi_c || to->type == Branch_c || to->type == Fork_c || to->type == LazyFork_ ||
...
std::string printMemEdges(std::vector<ENode*>* enode_dag, bool fix_mc_st_interfaces_flag) {
    string memstr = "";
    ...
    for (auto& enode : *enode_dag) {
        if (enode->type == MC_ || enode->type == LSQ_) {
			...
			for (auto& succ : *(enode->CntrlOrderSuccs)) {
                if (succ->type == End_ || ( (succ->type == Inst_ && isa<StoreInst>(succ->Instr)) && fix_mc_st_interfaces_flag) ) {   // AYA: 11/07/2023: commented the following condition since now the succ of the MC is ST 
                    ...
                    if(succ->type == Inst_ && isa<StoreInst>(succ->Instr) && fix_mc_st_interfaces_flag) {   // AYA: 11/07/2023: added the following extra condition to correctly index the input to ST            	
                    	// use its index in the CntrlOrderSuccs after adding 1 to offset END (I'm assuming that END is always at idx 0)
                    	auto pos = std::find(enode->CntrlOrderSuccs->begin(), enode->CntrlOrderSuccs->end(), succ);
                    	assert(pos != enode->CntrlOrderSuccs->end());
                    	int idx = pos - enode->CntrlOrderSuccs->begin();
                    	memstr += ", from = \"out" + to_string(idx + end_ind) + "\"";
	                    memstr += ", to = \"in" + to_string(succ_ind + succ->CntrlPreds->size()) + "\"";
	                    memstr += "];\n";
                    } else {
